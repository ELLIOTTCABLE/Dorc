...
1. README, DESIGN, IMPLEMENTATION, and TODO are human-written; USER_STORY, KNOBS, ANALYZER-NEEDS, AID-NEEDS, FORFEITS and TODO-ADDTL are LLM-generated but reviewed/maintained and relatively authoritative.
...
- the `Research/notes/311`-series identity model writes `mSort` and `mKey` for what every earlier document, and the spike's `kind__` members, call a *kind* and an *entity*; the key another key is looked up in is an `mKey-CatalogStore`, the key it is identified in an `mKey-PrimaryStore` (the sort-level views are `mSort-CatalogStore` / `mSort-PrimaryStore`). A hyphenated `mSpecies-Gloss` is a derived view of the species before the hyphen, never a declared species. Read a pre-311 "kind" as an mSort and a pre-311 "entity" as an mKey; the model never uses a bare English noun for these. Do not annotate the older spellings in passing ("foo, née bar"); this bullet is the bridge.
...
- run things through `mise run <task>`; `mise tasks` lists them, and `dir`/env come with the task, so it works from anywhere in the tree. add a task rather than re-deriving an invocation twice.
  - `mise run` (or `hk`) is *almost always* preferrable to hand-rolling one-off commands. collaborate and pay back: if tooling chafes, *fix the tooling* for future agents, don't work around it. if not authorized, report upwards, don't swallow. tooling matters.
  ...
  - `mise run both gate:full-quiet` is the primary cross-platform acceptance runner while you work; it leans toward terseness without hiding failures. For conductors, at the end, when landing large arcs of work, there's `mise run gate:arc` (~20GB RAM, substantial disk, multi-platform, substantial wallclock.)
  ...
  - *do not*, under absolutely any circumstances, use `git commit --no-verify`. `DORC_KNOWN_BROKEN` is the only authorized route for committing breakage. If there's broken tooling, fix the tooling. Be *careful* fixing tooling, it is a legitimate, high-priority side-quest, and isn't to be tossed out in passing - escalate to your conductor or the human if you are unsure.
...
- use your harness's built-in tooling: read/edit/write/grep/glob and similar; don't use Bash() to simply `cat` or `sed`/`grep`. Those tools cost tokens when you typo a command, and bypass harness protections.
  ...
  - this is *not* a ban on those tools, it's a ban on using `cat foo.rs` as a replacement for `Read(foo.rs)`; you should absolutely still reach for powerful tooling when you have a task that would *benefit* from them (e.g. general find-and-replace; complex-but-mechanical edits you can automate.)
