...
- run things through `mise run <task>`; `mise tasks` lists them, and `dir`/env come with the task, so it works from anywhere in the tree. add a task rather than re-deriving an invocation twice.
  ...
  - `mise run gate:full-quiet` is the primary test-runner while you work; it skips resource-intensive tests, and leans towards terseness. it's *not* an acceptance suite, if your conductor gives you another one; for conductors, at the end, when landing large arcs of work, there's `mise run gate:arc` (~20GB RAM, substantial disk, multi-platform, substantial wallclock.)
