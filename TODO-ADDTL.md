...
* [ ] **same-role-name-collision-ruling (`293:§5`)** — two separately-published oracle files may both define one role function (deliberate: two kind-owners describing one command; 5 fixtures ship it). Resolution today is first-match-in-load-order, and the shipped artifact REDEFINES the function immediately before each call site — sound only under strictly-linear execution; named breakage: probe-phase parallelism, apply-time guard interleaving, sourced-oracle idioms — and it contradicts `reserved.rs`'s munge-collision refusal. Three options named (`flat-name-collision-fails-fast` = human's stated lean · `ambiguity-fails-fast` · `one-function-per-provider-by-construction`); human-deferred "for a focused sitting" 2026-07-28, but the sitting was never scheduled and no register carried it until the 2026-08-13 catch-up → `notes/293` §5.
