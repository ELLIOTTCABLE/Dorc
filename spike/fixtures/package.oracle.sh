...
# operand as `sm.dorc.Package`; the `[ "$2" = "" ]` guard refuses a SECOND operand (a
...
apt_get__predict() {
   ...
   if [ "$2" = "" ]; then
      case $verb in
         install|reinstall) dpkg-query -W -f='${Status}' "$pkg" >/dev/null 2>&1 : sm.dorc.Package:"$pkg"@installed ;;
         purge|remove) dpkg-query -W -f='${Status}' "$pkg" >/dev/null 2>&1 :! sm.dorc.Package:"$pkg"@installed ;;
      esac
   fi
}
...
# `sm.dorc.Package` and the trailing mark carries the establish claim.
dpkg__predict() {
   case $1 in -i) shift ;; esac
   pkg : sm.dorc.Package = "$1"
   dpkg-query -W -f='${Status}' "$pkg" >/dev/null 2>&1 : sm.dorc.Package:"$pkg"@installed
}
