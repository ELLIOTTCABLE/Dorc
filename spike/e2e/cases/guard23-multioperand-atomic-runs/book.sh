...
# probe ships, no path through the check-body is reached, and no witness forms — the whole
...
# CHECK-BODY HAZARD, pinned by this floor (Research/notes/23A): this oracle's refuse-path
# exits 0 (`if [ "$2" = "" ]` false => rc 0), so a build that shipped the check as a guard
