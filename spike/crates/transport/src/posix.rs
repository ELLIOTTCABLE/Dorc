//! The ONE answer to "where is a POSIX shell on this machine" (`one-shell-answer`).
//!
//! It lives beside the local driver — the crate's own consumer — because a second resolver is how
//! the first one silently rotted. This crate already spawns processes to reach a shell, so resolving
//! which shell to spawn belongs here; nothing above it re-derives git's userland.
