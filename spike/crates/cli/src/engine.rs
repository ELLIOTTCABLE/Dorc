...
fn run_status(
    ...
    // The ORIGIN round decided which sites ship a verdict body and the probe is never rebuilt, so
    // this is frozen alongside the rest of the settlement's inputs rather than re-derived per round.
    ...
    debug_assert!(
        settled.quiescence.is_quiescent(),
    ...
    // executable view is final. Nothing is emitted yet — the stream write stays at its own seat.
    let artifact =
    ...
    // merely truthful.
    let planned_image =
        crate::apply::image_of_artifact_set(&artifact, &dorc_receipt::limits::ReceiptLimits::V1)
            .ok()
            .map(|image| image.id());
    ...
    let invocation = edges.invocation_record(InvocationRecordRequest {
        framing: &framing,
        snapshot,
        started_at,
    ...
    let presentation = dorc_plan::presentation::FinalPresentation::of_settled(
        ...
        dorc_plan::planning_input::PlanningInputs::of(
            crate::receipt_edge::CONTROLLER_SEMANTICS,
            &invocation,
            spine.admission(),
            admitted_records.as_ref(),
            dorc_plan::planning_input::PlanningPolicy::of(
                crate::receipt_edge::planning_mode(mode),
                options.risk_faultless_skips(),
            ),
        ),
        planned_image,
    );
...
fn record_new_arm(
    ...
    use dorc_core::spine::{
        AdmissionOutcome, ShipLane, SpineAdmission, SpineProbeShip, SpineSolveCertification,
    };
