...
/// whole-product receipt cases whose code fires on a later block, now that the inventory spans
/// every editable block (`30Xa:open-every-block-vars-reverted`). A floor, never a count: the corpus
/// drifts. No case is tolerated any more — a case the seat cannot inventory is a real gap, not a
/// silence.
#[test]
