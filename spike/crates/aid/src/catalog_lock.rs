...
pub const CATALOG: &[CatalogEntry] = &[
    ...
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm leaf-exact render refuses to {{verb}} a heredoc-bearing command (`{{command}}`): its span covers the `<<` operator, not the body lines, so substituting it would strand the heredoc body -- it runs verbatim")),
        help: HelpRegister::Written(ProseTier::Migrated("sm split the heredoc body to its own leaf, or mark the kind un-elidable")),
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm command `{{inner}}` runs inside a `$(...)` substitution => effect-bearing but not independently elidable (it runs whenever its enclosing line runs)")),
    ...
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm write-redirect to a dynamic/unresolved target => no per-path `file` cell can be keyed, so the write joins ⊤ and the command runs (never elided)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm call `{{name}}` not inlined: its argument references a positional (`$1`..`$9`/`$#`) that does not thread through two inline levels => the inner body's positional is ⊤ -- it runs as an ordinary unmodeled command (depth-2 positional threading is out of the modeled subset)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm errexit state is unknown at one or more commands; failure-edges added conservatively (over-approximate, sound)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm function `{{name}}` shadows a shell builtin the engine relies on (dash resolves a function before a regular builtin): analysis treats the bare word `{{name}}` as the builtin when classifying effects and minting stand-ins, so builtin-dependent conclusions may be unsound for this book")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm check annotation kind `{{annotated}}` disagrees with the effect-map kind `{{effect_map}}` for this verb -- the annotation (declared identity) wins")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm this file uses a dorc-lang dialect construct (a bind `name : kind = ...` or a trailing `:`/`:!`/`:?` mark) but lacks the `# dorc-lang/v0.2` version marker (marker-gates-syntax-only): add `# dorc-lang/v0.2` as a standalone comment in the first 10 lines, or drop the dialect (the bare `__role` floor works markerless)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm `{{token}}` is not a known context dimension on a `tolerates:` vouch (expected one of {{expected}}); the mark vouches nothing and the site stays walled on that dimension (`27C` section 2).")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm this `is_converged` carries `tolerates:user` but VISIBLY reads the caller's identity (`id`/`$USER`/`$HOME`): are you sure the body is read-only under a user shift, not just answer-varying? A shifted user must not make it MUTATE (`27C` section 2 corroboration).")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm this `is_converged` reads the caller's identity but carries no tolerance vouch -- a wrapped site (`sudo ...`) will run/guard instead of eliding. One line makes it context-shiftable: `: safe-across user` (`27C` section 2).")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        params: &["token", "expected"],
        example: "sm `frobnicate` is not a known lend_map dimension (expected one of user, fs-view, netns); the line mints no lend and the dimension it meant to answer stays ⊤ (walls). Dimension marks are an engine-owned closed vocabulary (`273` section 8).",
        message: Some(ProseTier::Migrated("sm `{{token}}` is not a known lend_map dimension (expected one of {{expected}}); the line mints no lend and the dimension it meant to answer stays ⊤ (walls). Dimension marks are an engine-owned closed vocabulary (`273` section 8).")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        params: &["kind_munged"],
        example: "sm `invariant:netns` is forbidden on the per-netns `net-kernel` store of `sm_dorc_Route` -- network kernel state is namespaced, never netns-invariant",
        message: Some(ProseTier::Migrated("sm `invariant:netns` is forbidden on the per-netns `net-kernel` store of `{{kind_munged}}` -- network kernel state is namespaced, never netns-invariant")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm verdict and observe marks are single-cell; brace alternation `@{a,b}` is claim-emission-only (`277` section 4c) -- this mark mints NO cell and the site will run. Split it into one marked probe line per cell.")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm fact lane truncated: {{received}} of {{declared}} declared site records received -- the {{unseen}} unseen site(s) fold Unknown (run)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        params: &["site", "call"],
        example: "sm site 0: touches() escalated to host-derivation (hork.touches())",
        message: Some(ProseTier::Migrated("sm site {{site}}: touches() escalated to host-derivation ({{call}})")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        params: &["site", "reason"],
        example: "sm site 0: derived footprint family incomplete (no deriv-end close-record) -- footprint refused, the site walls total (an at-most claim cannot be partial)",
        message: Some(ProseTier::Migrated("sm site {{site}}: derived footprint family incomplete ({{reason}}) -- footprint refused, the site walls total (an at-most claim cannot be partial)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm pure-predicate carry across {{axes}} (unflagged, 27C section 4(a)): {{kinds}} -- each vouched invariant across {{axes}} by its kind-owner's `invariant:` line (vouch-species); the verdict body is engine-proved read-set-closed")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm would elide if {{provider}}'s oracle vouched context-tolerance (one line: `: safe-across {{dimension}}`)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm kind '{{kind}}' has {{count}} resolvers across oracle files -- at-most-one-resolver-per-kind (24F section 3): BOTH refused, the kind keeps token-equality (never first-wins-silently)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm resolver '{{name}}.resolve()' is keyed to a name matching a known COMMAND provider -- resolvers are keyed by KIND, not command (corr-kind-keying section 10); this mints identity for a kind no coordinate may use (a likely mis-key)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm coordinate {{coord}} resolved DANGLING -- the kind's resolver reports no such entity (a likely typo / stale name); it degrades to may-alias (the site runs)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm kind '{{kind}}' has {{count}} reach-functions across oracle files -- at-most-one-reaches-per-kind (24G section 4): BOTH refused, the kind's footprints do not expand (never first-wins-silently)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm reach-function '{{name}}.reaches()' is keyed to a name matching a known COMMAND provider -- reaches is keyed by KIND, not command (24G section 4); this expands a kind no coordinate may use (a likely mis-key)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm this verdict arm authors a deliberate decline classed `{{class}}` (a `decline {{class}}` report emission) -- the site will run; the class routes the enhancement nags (advisory only).")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm this verdict arm authors a deliberate decline whose class is not statically readable (a dynamic format or an unrecognized class token) -- the site will run; the class resolves at runtime (advisory only).")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm `{{tool}}` was not found on PATH -- its checks were skipped. Install it, pass --no-tools to silence this, or --require-tools to make it a hard CI error.")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm `{{tool}}` exited with status {{rc}} but produced no parseable findings")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm {{wall_count}} unmodeled ⊤-{{wall_word}} in this book; the first is here. Downstream commands (~{{downstream}} leaf site(s) after it) lose full-elision and fall back to runtime guards until each wall's tool has an oracle.")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm this __is_converged body answers with a pipeline's tail status (oracle-contract section 3): the exit status is the LAST stage's, not necessarily the tool-under-description's -- prefer a shape where the modeled tool produces the status directly.")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm no book given (a positional path or --book=PATH)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm strip needs a file path, got the flag `{{got}}`")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm strip needs a file path")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm unknown flag `{{flag}}`")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm unknown flag `{{flag}}` -- did you mean `{{suggestion}}`?")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm unknown mode `{{mode}}` -- did you mean `{{suggestion}}`?")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm usage: dorc-sh <script> [args...]")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm --expect-files: expected {{expected}} lintable file(s), saw {{found}} (scope drift)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm no lintable files given (operational, not clean)")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        message: Some(ProseTier::Migrated("sm --require-tools: required tool(s) missing from PATH: {{tools}}")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        params: &[],
        example: "[unwritten: helper-declaration-contested]",
        message: None,
        help: HelpRegister::Absent,
    },
];
