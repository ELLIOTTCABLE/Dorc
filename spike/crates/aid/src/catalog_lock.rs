...
pub const CATALOG: &[CatalogEntry] = &[
    ...
    CatalogEntry {
        ...
        params: &["verb", "command"],
        example: "Dorc cannot guard `apt-get install -y nginx <<EOF` as planned: rewriting the line would strand the heredoc body below its `<<` opener, so the command stays in place and runs verbatim.",
    ...
    CatalogEntry {
        ...
        params: &["count"],
        example: "sm 1 torn (no terminal token) record line(s) discarded (counted, never folded)",
    ...
    CatalogEntry {
        ...
        params: &["axes", "kinds"],
        example: "This elision carried across fs-view: Dorc proved the verdict body reads nothing beyond the site's own arguments and sm.dorc.KernelParam (invariant: line at kp.oracle.sh:5), whose owners vouch that state does not divide across fs-view.",
    ...
    CatalogEntry {
        ...
        why: "27R §8 delta-exit-trichotomy-sharpened - the adapter never interprets a foreign tool-rc beyond zero/nonzero (checkbashisms' additive 1|2|4 codes are the named trap), so an unexplained failure is reported as itself. Replay injects a raw process outcome through the lint runner edge.",
        params: &["tool", "rc"],
    ...
    CatalogEntry {
        ...
        why: "27R §4 - upstream drift may cost precision, never a crash and never silence, so the raw bytes ship as one opaque finding rather than being dropped. Replay injects raw tool bytes through the lint runner edge.",
        params: &["tool", "output"],
        example: "sm unrecognized `checkbashisms` output (raw): upstream format changed beyond recognition",
    ...
    CatalogEntry {
        ...
        why: "the flag surface is per-invocation, so accepting it elsewhere would imply a capability that mode does not have.",
        params: &["flag", "mode"],
        example: "The `--host` flag is only valid with dorc plan or dorc apply.",
    ...
    CatalogEntry {
        ...
        params: &["first", "second"],
        example: "The `--receipt-id` and `--receipt-last` flags cannot both be given.",
    ...
    CatalogEntry {
        ...
        example: "Dorc does not recognize the flag `--boook`; did you mean `--book`?",
    ...
    CatalogEntry {
        ...
        example: "`--expect-files` declared 12 lintable files, but Dorc saw 1; the lint's scope has drifted.",
    ...
    CatalogEntry {
        ...
        params: &["tools"],
        example: "`--require-tools` was given, but these lint tools are missing from PATH: shellcheck, checkbashisms.",
    ...
    CatalogEntry {
        ...
        params: &["which", "line"],
        example: "Dorc refuses to ship the plan: it carries a carriage return (first on line 1), and CRLF bytes fail on the remote host before any shell can run -- convert the line endings to LF and re-plan.",
    ...
    CatalogEntry {
        ...
        example: "This `foobar__is_converged` overrides the definition at foobar.oracle.sh:1 with nothing spelling the choice, so Dorc withholds every license from the `foobar` family and its sites run -- an `unset -f foobar__is_converged` between the two spells the override as chosen.",
    ...
    CatalogEntry {
        ...
        params: &["name", "sites"],
        example: "`yum__is_converged` is defined below 3 of its family's sites, and a definition licenses nothing above itself, so those sites run as if it were absent -- moving it above them is one cut-and-paste.",
    ...
    CatalogEntry {
        ...
        params: &["name", "prior"],
        example: "Two differing declarations of `_wombat_dest` are loaded (the earlier at b-helper.oracle.sh:2), and nothing can choose between same-named helpers, so Dorc withholds them all and the sites needing `_wombat_dest` run -- load a single source, or take custody of a merged copy.",
    ...
    CatalogEntry {
        slug: "plan-import-rewritten",
        when_fires: "the emitted plan's own `.` line names something other than what the book's `.` named — the bundle this run composed (`{verb}` = `repointed`, `{names}` = the artifact-relative path), or nothing at all because the bundle's bytes stand where the load did (`{verb}` = `inlined`, `{names}` empty). Minted per book-sited load the form re-says, spanned at the authored `.`. `{reason}` is the front-lift ladder's own condition word for WHERE this bundle stands (30P:rul-front-lift-is-the-planners-first-consumer) — a reason component beside the one code rather than a sibling code per tier (28L:rul-reason-enums-not-sibling-codes). Today it always names a kept-in-place condition: `ladder-unconsulted` where the ladder has not answered, `shape-unmeasured` where the `.` is outside the absorbable cell, `operand-not-explicit` where a computed operand is not ours to re-say (30P:rul-rewrite-permission-is-derived) — and that last one emits no rewrite at all, so it reaches this code only if a form ever re-says such a line.",
        why: "30Ng:rul-bundle-at-dorc-lang-boundaries (human-typed) grants Dorc exactly one rewrite over a plan it generated — where an import points — because a generated plan is a durable but not an OFF-RAMP durable. That grant is narrow, and the thing that keeps it narrow in practice is that every use of it is said out loud: rul-attention-honesty makes a load whose target moved precisely the class of change that must never be silent, and two-surfaces puts the account on the plan surface while the artifact carries the bytes. Note severity with no floor: the plan is exactly what the run intended, and the reader is owed the fact rather than a warning. Remediation register (help) wanted: none — there is nothing to repair; if anything is wanted it is a pointer at the artifact file the import now names.",
        params: &[],
        example: "[unwritten: plan-import-rewritten]",
        message: None,
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        slug: "computed-source-operand",
        when_fires: "a BOOK's `.`/`source` operand carries a command substitution or an arithmetic expansion, so its value is something only the target can produce and the load plane cannot name the file the `.` loads. analysis/funcenv.rs `evaluate_word` records the cause, cli/main.rs `load_head_notices` mints it, one per `.` line, spanned there; the same cause raises the run's outcome to `EXIT_LOAD_UNRESOLVABLE`, whole-run and pre-network. World-as-payload: the mint sits in the binary's own load-edge driver, past everything the in-process book pipeline composes.",
        why: "`30P:rul-floor-valid-text-never-parse-fails` (TYPED 2026-08-22) moved this outcome's TIER and kept the outcome. `. \"$(dirname \"$0\")/helpers.sh\"` parses and runs under `posh ∩ dash`, so the parser refusing it — which is what happened until now — threw away a whole run's diagnosis and aid over text the floor accepts. The site itself is an ordinary point havoc (`principle-unknown-source-is-a-point-havoc`): bindings unknown AT that line, the file not acquired, the load walls. What survives is the pre-network fail-fast, because a `dorc … && deploy` chain must still stop: the plan Dorc can build over a book whose dependencies it cannot name is much poorer than the author expects, and silently proceeding would spend the reader's attention on it. The sanctioned repair is `rul-static-predict-sites-loads` — an authored, statically-evaluable `dirname__predict` whose stdout is positively claimed, which the stdlib does not yet carry — and until then the EXACT spellings are `${0%/*}` and the `case $0 in */*)` fold. Remediation register (help) wanted: name those spellings, and say that the artifact still ships. The softer UX (per-site rather than whole-run, or no fail-fast at all) is the human's to rule; the machinery here handles the worst case either way.",
        params: &[],
        example: "[unwritten: computed-source-operand]",
        message: None,
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        slug: "oracle-matched-zero-sites",
        when_fires: "a LOADED oracle's `__is_converged` verdict never actually vouched a single site this run -- either its command word never appears anywhere in the book, or every invocation's argv declined. cli/main.rs oracle_matched_zero_sites_diagnostics, aggregated from the run's own final `Vouches` (never a second lift).",
        why: "30Qe:fruit-oracle-matched-zero-sites. The silent-decline value-evaporation detector (`KNOBS:kWARN` rich, tune-high): loading an oracle that never earns a chance to help is easy to do quietly (a typo'd pre-source path, a stale command word, a book that moved on) and easy to never notice, since nothing breaks. `{oracle}` carries the loaded oracle's own path. World-as-payload (`aid/CLAUDE.md` fixture tier): the trigger reads the run's WHOLE final vouch set, which this single-file consumer has no seat to reconstruct; the real firing route is proven separately by the whole-product round-trip case `crates/cli/tests/oracle-matched-zero-sites-round-trip.loom`.",
        params: &[],
        example: "[unwritten: oracle-matched-zero-sites]",
        message: None,
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        ...
        when_fires: "a `dorc apply --host` invocation could not turn the bytes it was handed into something a dispatch permit may be minted over, so it shipped nothing. cli/main.rs apply_refused and apply_edge_refused, over cli/apply.rs consented_apply's own refusal and over the local durable edge that would not open. {reason} is the closed word for which step did not close: `image-not-recordable` (the bytes are past what one exact image may carry), `session-not-preparable` (the session could not bind its own assignment), a `receipt-not-*` word (the document the store would not take), or a root/key/store word straight off the local edge (`no-controller-root`, `store-not-a-directory`, a keyset state, ...). {store} is the per-user state base this apply would have filed under. The edge words are reachable from the shipped binary and covered at cli/tests/receipt_state.rs; the first two are not, which is why this case drives the harness defect route.",
        why: "30Rb apply-dispatch-sequence - an intent binds EXACT bytes, so bytes that cannot be recorded exactly are bytes no permit may be minted over and no dispatch may follow. One code carrying a reason WORD rather than sibling codes (28L rul-reason-enums-not-sibling-codes): the world is one - an apply that bound nothing and shipped nothing - and only the step that did not close differs. The word is the most specific one each route has, because rounding them all to one `intent-not-published` named the step everyone could already see and dropped the only thing a reader acts on (30Rs fix-apply-durable-reporting); the store rides beside it because the same word means different repairs at a per-user root and at a named one. Remediation register (help) wanted: per reason, and for the image bound that means naming the bound rather than the file.",
        params: &[],
    ...
    CatalogEntry {
        ...
        params: &["store", "reason"],
        example: "this run finished and its artifact is unaffected, but no signed receipt reached <no state root> (store-unusable) -- there is no durable for a later `dorc why` to read",
        message: Some(ProseTier::Slop("this run finished and its artifact is unaffected, but no signed receipt reached {{store}} ({{reason}}) -- there is no durable for a later `dorc why` to read")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
        ...
        params: &["store", "reason"],
        example: "no receipt could be read at <no state root> (no-controller-root) -- either none has ever been written here, or the store would not open; nothing is removed or repaired on this path",
        message: Some(ProseTier::Slop("no receipt could be read at {{store}} ({{reason}}) -- either none has ever been written here, or the store would not open; nothing is removed or repaired on this path")),
        help: HelpRegister::Absent,
    },
    CatalogEntry {
    ...
    CatalogEntry {
        slug: "apply-outcome-unwritten",
        when_fires: "a `dorc apply --host` published its intent and touched the machine, then could not record its outcome. cli/compose.rs dispatch_and_report_apply, over cli/apply.rs consented_apply's post-dispatch outcome publication. {intent} is the surviving intent id the user keeps for a later `dorc why`; {step} is the closed word for which durable act of recording the outcome did not close.",
        why: "AID-NEEDS:law-codes-vary-by-world-not-grammar. A SIBLING of durable-receipt-unwritten, not a reason arm widening it: that is the plan-time world where nothing was touched and the repair is to re-plan; this is the post-dispatch world where the intent WAS published, the machine MAY have changed, and the repair is to check the host and keep the intent id. 30X section 11 (the post-dispatch surfaces); 30Rs fix-apply-durable-reporting. Remediation register (help) wanted.",
        params: &[],
        example: "[unwritten: apply-outcome-unwritten]",
        message: None,
        help: HelpRegister::Absent,
    },
];
