...
pub const CATALOG: &[CatalogEntry] = &[
    ...
    CatalogEntry {
        ...
        params: &["name"],
        example: "sm call `install_one` not inlined: its argument references a positional (`$1`..`$9`/`$#`) that does not thread through two inline levels => the inner body's positional is ⊤ -- it runs as an ordinary unmodeled command (depth-2 positional threading is out of the modeled subset)",
