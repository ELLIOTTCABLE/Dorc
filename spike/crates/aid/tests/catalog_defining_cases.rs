...
fn unwritten_renders_are_greppable_and_pinned() {
    ...
    // nothing readable, and a store whose greatest order names a cohort + a deliberate one-line
    // bump for `apply-receipt-not-optional`, the remote apply's refusal of `--no-receipt`.
    // Unwritten because a builder authors ZERO prose (`27V:rul-error-authorship-tier`); each
    // carries a `why` naming the remediation register its words are owed.
    assert!(
        unwritten.len() <= 25,
