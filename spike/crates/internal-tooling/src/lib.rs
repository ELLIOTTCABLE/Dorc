...
//!
//! Three library exports. [`Posix`] is the answer to "where is a POSIX shell on this machine",
//! computed once, explicitly, from a source we already depend on. [`xfail`] is the workspace's ONE
//! xfail-pin seat and its census; [`run_seed`] is its ONE run-seed seat — "what is this run's seed",
//! drawn once, printed and named the same way everywhere. Both are test scaffolding rather than shell
//! plumbing, but sited here for the same reason: every crate can dev-depend this one, and a second
//! copy is how the first silently rots.
