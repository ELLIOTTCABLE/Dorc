...
fn header(fingerprint: &str) -> String {
    format!(
        "# SLUGS — generated index of every slug the corpus defines or cites\n\
         ...
         row: `grep -C5 touches-becomes SLUGS.md`. Each row is one slug. Machine-generated; do not edit.\n\
         \n\
         ...
         defined. `near:` lines are advisory, computed from {fingerprint}, and refresh only when the\n\
...
fn render(
    ...
    let mut out = header(fingerprint);
...
fn parse_previous(text: &str) -> Previous {
    ...
    for line in text.lines() {
        if let Some(slug) = line.strip_prefix("## ") {
            current = Some(slug.trim().to_owned());
...
pub(crate) fn run(args: &[String]) -> ExitCode {
    ...
    let rendered = render(&rows, slug_near::FINGERPRINT, &near);
