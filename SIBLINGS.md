...
> not a scorecard: a **Y** under Dorc is a choice you may or may not want, and a
> **Y** under anyone else is a strength worth having — usually bought with a cost
> that is also in the table. Its standing job is to keep three things visible
> while we work: what we are not trying to own, what we cannot do well, and which
> users we should push toward a better tool for them.
>
> Rows are architecture — fundamental decisions and their lock-in/lock-out —
> never gaps either side could close with ordinary implementation work. Library
> size and maturity are deliberately absent (Dorc's answer today is "young and
> nearly empty"; that line belongs to the README's own voice, not a table).
>
> Legend: **Y** yes, as a fundamental capability · **N** no, as an architectural
> decision · **~** partly, see that tool's notes · **—** not that tool's layer.
> Long-form evidence: `Research/notes/r26-glue-strawmen/SIBLINGS-fragment-*.md`
> and the round ledger under `.claude/research/ops-glue-residue/`.
...
| Your existing shell scripts usable on day one | Y | ~ | ~ | N | N | N | N | Y |
| Convergence machinery for raw shell content | Y | N | N | N | N | N | N | N |
| Check-then-converge built into its own units | — | Y | Y | Y | Y | Y | Y | N |
| Preview before mutating (plan / dry-run / diff) | Y | ~ | ~ | ~ | Y | Y | ~ | N |
| What you review is byte-for-byte what executes | Y | N | N | N | N | ~[^k1] | Y | N |
| Plan artifact runs without the tool installed | Y | N | N[^p2] | N | N | N | N | — |
| Off-ramp: stop using it, keep working artifacts | Y | N | N | N | N | N | N | ~ |
| Skips explained with queryable provenance | Y | N | N[^p1] | N | — | — | — | N |
...
| Fleet: inventory, groups, per-host data | N | Y | Y | ~ | Y | Y | ~ | — |
| Creates infrastructure (VMs, DNS, networks) | ~ | ~ | ~ | N | Y | ~ | N | N |
| Whole-system rollback | N | N | N | N | ~ | ~ | Y | N |
| Privilege escalation built in | N[^d2] | Y | Y | ~ | — | — | ~ | — |
...
niche; choosing among us is a legitimate matter of taste and situation.
**The channels** — cloud-init and the OS installers — are neither: they are how
...
- Its history is generous with lessons: facts are gathered before execution, so
  ...
  the staleness is *shown* (the plan you consent to), and the default fallback
  is a runtime guard rather than an opt-in flag.
...
- Its PreOS feature answers first-boot by *manufacturing the ssh precondition*
  (a bootable image with your key baked in); we answer it by compiling
  convergence into the payload. Both coherent.
...
- What you should use for anything that is a *resource*: machines, networks,
  DNS, load balancers. Dorc's pivot books can drive a cloud CLI for a box or
  two, and are the wrong tool by the seventh dependent resource.
...
- Not a choice, and the residue around it is unusually crisp because Kubernetes
  ...
  *health*, never the *content* of the shell inside init containers and hooks —
  it mandates that content be idempotent and, for init containers, rejects
...
- It is also a genuine sibling deployer (`nixos-rebuild --target-host` pushes
  ...
  Our oracle *consumes* their soundness. (Their `dry-activate` self-documents
  as incomplete; a tool can have a sound convergence check and an unsound
  dry-run — different questions.)
...
- **cloud-init** runs where no push tool can exist yet: first boot, no inbound
  ...
  code, never secrets.
- **The OS installers** (autoinstall, preseed, kickstart) are one-shot by
  shape — the machine leaves the channel forever, and nothing in the hook is
  assisted. A compiled book dropped there gains a day-N future the channel
  itself can't offer. Notably it is also the one channel that *should* consume
  our exit code (non-zero aborts the install) — producing a meaningful rc and
  refusing to trust a channel's rc are different things.
- **Ignition** states the principled opposite of our bet: modification means
  ...
  all." Where you can truly re-provision on every change, take their axiom — it
  is better than ours. Three coherent positions on one axis: Ignition refuses a
  half-machine, cloud-init produces one and reports done, Dorc produces one,
  names what is missing, and finishes next run. **Talos** (no shell at all) is
  fully, permanently conceded.
