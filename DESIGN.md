...
Presenting a *plan* to the user, though, means we don't have the apply-phase 'convergence floor' for the probe-phase; it's relatively performance-forward, as the user is sitting there, waiting on a coherent plan (i.e. for all the probes to finish and return), before they can hit 'submit' and go off and do something else in the worst/slowest case. We can't be depending on multiple runs for truth. Further, we don't have a guarantee that the user will *accept* the plan - by the very act of presenting a plan and giving them an option, we're implicitly promising to *not* pre-apply parts of the presented plan. (i.e. providing plan/apply limits the "safety fallback" of probe-causes-mutation; even probe-causes-maybe-eventually-*desirable*-mutation is, sadly, verboten.)
