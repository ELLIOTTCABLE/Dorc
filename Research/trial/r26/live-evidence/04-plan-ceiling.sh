...
true || :   # dorc: elided [dpkg -s ca-certificates >/dev/null 2>&1; apt-get install -y ca-certificates] (already converged / dead branch)
true || :   # dorc: elided [dpkg -s curl >/dev/null 2>&1; apt-get install -y curl] (already converged / dead branch)
true || :   # dorc: elided [dpkg -s jq >/dev/null 2>&1; apt-get install -y jq] (already converged / dead branch)
