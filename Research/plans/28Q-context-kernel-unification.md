...
8. `an-atmost-completion-signal` (exit-0 truncation) — RULED at `30U` §5: the
   completion witness is the `disturbs nothing-else` report-lane record, mandatory
   only where licensure rides runtime emission (dynamic `disturbs` bodies) or as the
   finished-definition act in `disturbance_reaches` bodies, and absent by design for
   collide-only emissions. §3's atomicity inheritance is its first conjunct, not its
   closure. Build rides the stdlib.
