...
**Status: live constraint on the core tool, NOW.** (Companion: `deferred/privileged-tracing-tool.md` = the eventual tool that consumes this surface; that one is deferred. This one is not — these are things the core must *not optimize away* from day 1, because retrofitting them is the kind of lock-in the perf round warned about.)
