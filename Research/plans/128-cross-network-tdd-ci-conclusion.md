...
Round 12, 2026-06-03 (rev. after human review: the seam model is corrected — see se-1; the
round-12 `notes/` keep the superseded framing as historical record, per AGENTS.md notes-vs-plans).
Synthesized from the front notes (`notes/120` broad sweep; `notes/122` DST explainer; `notes/123`
Rust ecosystem; `notes/124` seam-onto-Dorc; `notes/125` containerizability quadrant; `notes/126`
transient-fault) and the human's direction in `notes/127`'s ledger. The round-12 map (`plans/121`) is
frozen and not rewritten here. 33 sources registered this round (148 total, `../sources.json`); the
DST / Jepsen / IaC-self-test / tier-theory clusters were subagent-graded on the broad sweep and
re-verified on full read when narrowing.
...
- **concl-seam — Fronts A and C unify at one controller↔host *session* boundary, and reserving it is
  the only decide-now piece.** +SURE the unit that crosses the network is a *whole compiled script /
  program per host* (probe = flat-ish; apply = the full CFG), ~O(hosts) tunnels — **not** a per-leaf
  call. (Round-11 `plans/111` dac-C and `DESIGN.md`: a leaf "is *not* a network op; it runs as `sh`
  inside the streamed batch"; `(host, leaf)` is a *compile-time* analyzer coordinate, not a runtime
  RPC.) So the single all-nondeterminism seam is the **transport / session boundary** —
  `ship(host, unit) → results` — and DST hooks there by swapping the production transport for a seeded
  result-synthesizer. This is *distinct* from `plans/077`'s **host-side** per-leaf wrap
  (trace / `DORC_LEAF_ID` / seccomp, running inside the shipped unit): two seams at two tiers, linked
  only by the marker/metadata protocol the transport carries (§3 se-1). Reserving the transport seam
  cleanly (rung L0) + keeping the orchestrator kernel injecting clock/rand/IO is the only
  retrofit-hostile, day-1 item; DST is "design-it-in-or-never" [B-antithesis-dst-primer-2024]. **The
  provisional lean (human, not a decision) is to bank exactly this and write the kernel
  async/inverted-control;** climbing higher is deferred (§3). What metadata-*form* crosses the seam —
  in-band transpiled markers vs a bootstrapped probe-executor's OOB channel — is the new `kCOMMS`
  knob (OPEN); the seam-bank is robust to it (§3 rob-1).
...
- **Object A — how Dorc tiers its *own* tests.** This is **Rust**, tiered by **Rust** annotation (test
  ...
  *mostly-no, like everyone*. +SURE. The portable low-tier enforcer is the **DST transport seam
  itself**: synthesized results mean no real socket opens, so "no network" holds *by construction on
  every OS*. A Linux-only seccomp escape-detector can sit in CI as a backstop, never in the hot loop
...
> tests the primary, portable enforcer is the DST transport seam (qd, Object A).
...
**concl-seam, expanded.** The structural facts (Front A, `notes/124`, with the seam relocated per
human review):

- **se-1. The network unit is a whole compiled unit per host; the DST seam is the controller↔host
  *transport*, and it is distinct from `plans/077`'s host-side leaf-wrap.** +SURE. Round-11
  `plans/111` dac-C and `DESIGN.md`: Dorc ships a complete script (probe = flat-ish, apply = full
  CFG), ~O(hosts) tunnels (~2/host), and the controller reconstructs per-leaf facts from the
  *delimited host output* — a leaf is not a network op. So there are **two seams at two tiers**:
  - **Seam-1 — controller-side transport/session:** `ship(host, unit) → results`. The DST/fault
    *substitution* point, and cleanly mockable **in-process** — in simulation you *synthesize* the
    returned results from a seed; you never spawn `ssh`.
  - **Seam-2 — host-side leaf-wrap (`plans/077`):** each leaf wrapped `<wrapper> -- <leaf>` with
    `DORC_LEAF_ID` / optional seccomp, running *inside* the shipped unit; "process-level" precisely
    because it spawns `docker`/`apt`. Seam-2 *emits* the per-leaf marker/metadata protocol that Seam-1
    carries; the two are linked only by that protocol.

  This **corrects a per-leaf-network-call framing (`dorc_exec(host, leaf)`)** that crept into the
  round-12 DST notes (`notes/122` strawman, `notes/124` f24/f27) — superseded here; those notes stand
  as history. The earlier "process-level vs in-process is the whole crux" claim dissolves: the DST
  seam is Seam-1 (mockable in-process), and "you cannot run `apt` in the simulator" is a *non-issue* —
  `apt` runs on the host, which is the mocked edge by design (se-2). [A-fdb-sigmod-paper-2021]'s
  "the production implementation is a simple shim to the relevant system calls" is exactly Seam-1.
- **se-2. What DST CAN vs CANNOT test for Dorc.** +SURE. CAN: the orchestration kernel under network
  ...
  before the ack" (where the controller's knowledge of which leaves ran is bounded by the last marker
  received before the drop → a verdict over a *leaf-range*, fc-2). That is precisely Dorc's
  *differentiating* correctness vs Ansible. CANNOT: whether the opaque program actually worked, or
  real-host behaviour (the user's own sshd-renegotiation example) — the mocked edge (`notes/124` f25).
- **se-3. DST is unusually CHEAP for Dorc — and the seam correction *strengthens* this.** ~SUSPECT
  (human verdict, `notes/124`; `notes/123`). The relocated seam is a clean controller-side transport
  boundary, trivially mocked by synthesizing results — not the messy "run external programs in a
  simulator" the per-leaf framing implied. Three standing reasons: (a) Rust's swap story is **zero
  production cost** — `cfg`-conditional shim crates, `[patch.crates-io]`, libc-symbol overrides, all
  identical to the originals in a normal build [A-madsim-deterministic-simulator-2025]
  ...
  async-native developer and independently warranted elsewhere in Dorc (Menhir-style IoC parser
  error-handling); (c) **the transitive-dependency wall is *smaller* for Dorc than for a DB** — the
  analyzer kernel is near-pure (AST→facts), and Dorc's most dangerous "dependency," the remote host,
  is not a crate to `[patch]` but **THE seam you mock by design** (`notes/123` f23, `notes/124` f30).
  The state-machine architecture (sled / Polar Signals — deps-as-messages, fault-injection in the bus
  only) sidesteps the wall entirely [A-sled-simulation-guide-2020][B-polarsignals-dst-state-machines-2025].
  Residual cost ≈ one upfront seam choice + crate-gating that bites *only* kernel-reachable deps.
- **se-4. `kFIDELITY` gains a third mode.** +SURE. `plans/077`'s optimized-vs-faithful becomes
  optimized / faithful / **simulated**, where *simulated* synthesizes what crosses Seam-1 (the result
  stream / messages), no real exec. The faithful mode's "one leaf, one execution, control-flow
  preserved" 1:1 leaf↔source mapping is a Seam-2 / transpilation property — it *shapes* the marker
  protocol Seam-1 carries — complementary to, not the same as, the Seam-1 substitution point
  (`notes/124` f29).
- **rob-1. The L0 seam-bank decouples from the transport / `kCOMMS` decision.** +SURE. Whatever
  crosses Seam-1 — a marker-laced shell stream (`kCOMMS-transpilation-inband`) or a bootstrapped
  executor's OOB messages (`kCOMMS-executor-OOB`, agentless-but-not-executorless, à la Ansible's
  python-bootstrap) — the simulator synthesizes *that* from the seed. So banking L0 presupposes
  neither `kCOMMS` pole nor the orchestrator-ownership question; it can be done before either is
  decided. (Apply leans `kCOMMS-transpilation-inband` regardless, to preserve the
  `ssh host dash -s <script` off-ramp; the live choice is sharpest for the probe.)

**The `axis-dst-cost` ladder (laid out for the deferred decision — NOT decided):**

- **L0 · Reserve-the-seam — the day-1 bank (provisional lean: yes).** Make the controller↔host
  **transport/session** a clean trait/message boundary (Seam-1) and keep orchestrator logic injecting
  clock/rand/IO. Distinct from `plans/077`'s host-side wrap (Seam-2), though both are "reserve a clean
  indirection" commitments. Cost ≈ nil; buys DST staying *possible*; the only retrofit-hostile piece;
  robust to `kCOMMS`/transport (rob-1). Serves priorities #1/#2 with no maintainability tax.
- **L1 · In-process orchestrator sim — the plausible sweet spot, deferred.** A synthetic-result
  backend (synthesizing the Seam-1 stream/messages) + seeded scheduler fuzzing
  drop/timeout/partition/reorder/truncate/crash across the multi-host DAG, asserting invariants after
  every step, printing+replaying the seed (sled-recipe-sized [A-sled-simulation-guide-2020]). Tests
  the differentiating correctness (se-2 CAN).
...
- **se-5. THE FORK the human must weigh (adversarial, `notes/124` f26) — sharpened by the seam
  correction.** DST's payoff for Dorc ∝ how much genuine concurrency/ordering/failure-interleaving
  complexity lives in the **orchestrator** (DST-testable) vs at the **mocked edge** (oracle
  territory). The correction *raises* the edge's share: because the controller's per-leaf knowledge is
  mediated *entirely* by what crosses Seam-1 (markers / OOB metadata, `kCOMMS`), even more of "did it
  actually work on the host" sits at the mocked edge. The DST-testable kernel is the *coordination*
  logic — transport management, parsing a possibly-truncated result stream, assigning the verdict over
  the affected leaf-range, scheduling, fan-in — genuinely non-trivial under `kAGENTLESS`
  single-controller fan-in. So **DST is necessary-not-sufficient for Dorc**, and L1's value rests on
  that coordination layer being non-trivial. ~SUSPECT; this is the input to the deferred L0→L1+ rung
  decision, **gated by two OPEN facts** (ledger §7): if the transport becomes an
  agentless-temporary-executor channel (`kCOMMS-executor-OOB`), it is socket-shaped and madsim/turmoil
  apply directly; if the orchestrator is delegated (pyinfra or similar), the DST-testable kernel
  shrinks and se-5 re-resolves toward "less."
...
- **fc-1. concl-jepsen: prior-art-to-mine primarily; optionally a slow real-cluster CI tier with a
  *custom* checker; NOT the hot loop.** +SURE→~SUSPECT (`notes/126` f40). Jepsen's *shape* maps onto
  ...
  (c) it is the slow real-cluster tier, while the DST transport seam gives portable/deterministic/fast
  injection Jepsen cannot. Most reusable: the `:ok/:fail/:info` op model, the
  generator→nemesis→checker architecture, the fault catalogue, "prove presence not absence." Maelstrom
  fits *less* (Dorc is a controller, not a peer speaking a protocol over stdin/stdout)
  [A-maelstrom-distributed-systems-workbench-2022]. **Whether to actually adopt Jepsen/Maelstrom as a
  CI tier is DEFERRED** (ledger §7).
- **fc-2. Jepsen's op model IS the three-valued verdict — independent convergence with round 11.**
  ...
  verdict:** inject a timeout / mid-stream truncation at the **transport seam** (Seam-1) and assert
  the orchestrator records **Unknown** for the leaves *after the last received marker* — a leaf-range,
  not a single leaf — never a false ok/fail (`notes/126` f39). This is the CAP third state Dorc
  already committed to, and the transport seam is the natural place to *inject* and *assert* it.
- **fc-3. Fronts A and C unify at the seam (counter-thesis resolved).** ~SUSPECT→+SURE (`notes/126`
  f42). Synthetic adverse-outcome injection at the **transport seam** (Seam-1) —
  connection-fail / mid-stream truncation / delay / lost-results / reorder — is deterministic,
  portable, hot-loop, and tests the orchestrator's *logic*. Real netem/iptables/Jepsen only add value
  for *transport integration* (does Dorc's ssh-wrapping survive a real mid-stream drop?) and
  end-to-end assurance — which for Dorc is mostly the *mocked edge*, not Dorc's own code → a slow
  high-tier CI complement, not the primary. **Front C's apparatus ≈ Front A's seam + adverse outcomes
  + Sometimes-assertions + a custom checker.**
- **fc-4. Testing error-provenance under faults + the retry-safety hazard (bridge to round 11).**
  ...
  mid-stream-truncation-after-mutate, retrying, asserting no double-apply (idempotency as a *test
  oracle* here, even though idempotency-as-product is parked — td-3).
- **fc-5. "Is my fault-injection actually exploring anything?" — the half-answer is
  ...
  truncate-after-mutate / Unknown path is hit. The Rust ecosystem's determinism guard (rerun the seed,
...
**The portable fault-injection ladder (`axis-platform`, `notes/126` f44):** transport-seam-synthetic
(hot loop, all-OS, deterministic) → Toxiproxy userspace TCP proxy (real sockets, cross-platform,
CI-ish, cut-then-restore [B-testcontainers-toxiproxy-2024]) → `tc`/netem + `iptables` (real kernel
faults, Linux-only, slow CI [A-tc-netem-network-emulator-2026]) → full Jepsen (real cluster, Linux,
Clojure). The hot loop must live at the portable top; kernel-altitude is CI-only. Note the caveat that
local netem loss can be masked by TCP retransmit and reorder needs delay (`notes/120` f10) — a reason
the *synthetic* seam (which models the *outcome*, not the kernel mechanism) is cleaner for the hot
loop.
...
the transport seam) + write the kernel async / inverted-control — and defer whether to climb to L1+.
Language **leaning Rust** (the DST-maturity input, `notes/123` f22, is *minor* and the choice is
...
**OPEN (unresolved facts that gate later design).** Transport / comms model — now named `kCOMMS`:
"SSH a whole script over and parse in-band markers" (`kCOMMS-transpilation-inband`) is a *starting
point*, not a goal; an agentless-but-not-executorless model (bootstrap a temporary probe-executor
that only imitates shell but reports OOB, à la Ansible's python-bootstrap — `kCOMMS-executor-OOB`) is
live (`notes/124` Open-Q1). If the controller↔executor channel becomes socket-shaped, madsim/turmoil
apply directly. Orchestrator ownership — Dorc's own vs built-on-pyinfra/similar (if delegated, the
...
- *madsim's tokio-shim as Dorc's seam mechanism* — why-not: Dorc's host transport (Seam-1) is a
  whole-unit ship over ssh/session, not internal tokio sockets, so the tokio-shim covers only whatever
  *internal* controller networking exists; Seam-1 wants provider-trait or state-machine treatment
  (`notes/123` f21, f28). Re-opens **iff** transport becomes socket-shaped (`kCOMMS-executor-OOB`
  phones-home; OPEN).
- *Per-leaf network execution (the `dorc_exec(host, leaf)` framing)* — why-not: contradicts the
  decided ~O(hosts)-not-O(leaves×hosts) tunnel model (`plans/111` dac-C, `DESIGN.md`); leaves run as
  `sh` inside one shipped unit, so the seam is the session, not the leaf (se-1). Banked as the
  superseded framing the round-12 notes carry.
...
resolved) · C ✓ (transient-fault `notes/126`, Fronts A+C unified at the transport seam). Round-12
sources (33 new; the DST/Jepsen/IaC/tier clusters subagent-graded then re-verified on full read):
...
Cross-links: `plans/121` (frozen map) · `plans/077` (the **host-side leaf-wrap** seam = Seam-2;
trace / `DORC_LEAF_ID` / seccomp — *not* the DST transport seam) · `plans/078` (privileged tracing —
parked) · `notes/112` f50 + `plans/111` (three-valued verdict + the ~O(hosts) whole-unit transport
model, dac-C) · `notes/110`–`113` (error-provenance bridge, fc-4) · `notes/076` (network dominates
perf — `axis-quadrant`) · `KNOBS`: `kCOMMS` (transport metadata-form — in-band vs executor-OOB;
se-5/rob-1), `kFIDELITY` (third mode, se-4), `kAGENTLESS` (single controller = coordination
complexity, se-5), `kVERIFY-calibrate` (TypeScript-not-Coq = concl-rigor), `kVOLATILES-exclude`
(hermeticity = the determinism precondition DST shares), `kFAIL` (phase-keyed safe directions = the
three-valued verdict's grounding), `kSTATE`/`kSCHEDULE`/`kELISION` · `DESIGN.md` (soundness capped —
read via concl-rigor, not as a rigor-escape-hatch; whole-script transport) · `AGENTS.md`
