...
The guard-Query template (20E) licenses a command into the probe iff it is a **declared Query** — the oracle's `oracle_effect <provider> <verb> query <selector>` (`20E §0`, `crates/oracle/...` `Polarity::Query`). The same gate is the only sound one here: **the inner `$(cmd)` is probe-eligible iff `cmd` resolves (through its oracle's `predict()`) to a `CommandEffect::Queries(fact)`** — i.e. the oracle self-vouches it as read-only. +SURE this is the gate (it is the *exact* self-vouch carve-out the human welded: probe-inertness comes ONLY from structural vouching, `spike/CLAUDE.md` "Standing human rulings"; an oracle vouches its own command by existence, `DESIGN.md:505-512`).
