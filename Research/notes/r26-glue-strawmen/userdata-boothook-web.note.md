...
| Half-applied firstboot is the designed-for normal case | <https://docs.cloud-init.io/en/latest/explanation/failure_states.html>; and cloud-init reports `done` in cases including "cloud-init internally experienced an error" |
...
| User-data is IMDS-readable plaintext; Azure states the rule | cloud-init's EC2 datasource doc shows the literal `GET http://169.254.169.254/2009-04-04/user-data`; Azure: "We advise *not* to store sensitive data in custom data" — <https://learn.microsoft.com/azure/virtual-machines/custom-data> |
