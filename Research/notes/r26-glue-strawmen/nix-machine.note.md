...
- **flag-os-release-sourcing-is-the-densest-wall** — the sharpest one, and I would put it in front of the human. Turn C measured `. /etc/os-release` + `case "$ID"` as the single densest non-trivial construct in real glue scripts (~28k indexed files, an order of magnitude above wait-loops). This book's whole top region hangs off it: on NixOS the install branch is dead, and folding it is worth more than any elision further down, because install regions sit at the top of books and a wall there poisons everything.

  But `$ID` arrives by **dot-sourcing a host file into the current scope**, and `.` can define anything, so following it is not obviously in reach. If it is not, the densest idiom in the corpus is a wall, and every branch-on-distro book degrades to guards throughout.

  There is a shape of answer that does not require solving dot-sourcing in general, and I flag it rather than propose it: `/etc/os-release` is a *specified* format — freedesktop, shell-compatible `KEY=value`, no logic permitted. So it is a candidate for being modeled as a **read of a known file whose parse is contracted**, rather than as an arbitrary source. That would be an oracle-shaped answer to what looks like an analyzer-shaped problem, which is the direction this project usually prefers. Whether the general `.`-of-an-arbitrary-path stays a wall while this one specific, spec'd file does not, is a real design cut and not mine.

- **flag-nix-run-is-a-peeling-wrapper** — `nix run <flakeref> -- <cmd> <args>` parses a prefix of its own argv and execs the remainder, which is the peeling-wrapper tautology exactly. It is also the *documented bootstrap idiom* for standalone home-manager, so it appears in the first line of many real books. The book routes around it (bootstrap once, then call the tool by name) rather than inventing a `nix` wrapper-family oracle inside a book about something else — but the shape is common enough to want one, and `nix run`/`nix shell`/`nix develop` are three wrappers in one family.
