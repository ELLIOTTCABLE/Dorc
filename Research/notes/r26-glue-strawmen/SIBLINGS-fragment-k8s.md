...
| A general-purpose convergence predicate over object state | **Y** | delegates | Worth its own row because it is unusually strong prior art: `kubectl wait --timeout=0s` is documented as *"check once and don't wait"*, and `--for` takes arbitrary `jsonpath='{...}'=value` as well as named conditions. So Kubernetes ships one read-only verb that answers "is this predicate true right now" for **every resource type in the cluster**. Most incumbents ship nothing of the kind (the round found three mutually incompatible tri-state "nothing to do" rc conventions in the wild instead). An oracle arm for it is near-pure delegation covering a whole class — and it is the clearest instance in the table of a Big Boy having *already built* the thing we should never build. |
