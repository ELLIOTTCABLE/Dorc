...
gate, the index-kind context slot, and the mPlaceholder/witness machinery; change WHO answers the
...
(one NFS file) is exactly what makes a mCell-level `disjoint` unreachable for L3.
...
"keyed by K'" is derived from unequal store mTokens across K' (Service@active stored-in
...
`disjoint` resting on a perished mToken) collides.
...
A `resolve()` binds mPlaceholders at probe (`sk(E, K)` per mEntryChain); re-run at apply standup
and after every fired transit disturbance; mismatch is integrity (withhold), never a verdict
input (`30W` §4; `26Ob:cor-standup-witness-licenses-bare-line-elision`). Two mSites under one
mEntryChain share a mPlaceholder pre-measurement as today.
...
- L1@alpha, L3@alpha: File:/srv/people/accounts.db → mToken T. L2@beta, L4@beta:
  File:/mnt/team/people.db → mToken T (the shared export). Account's store set is total (one
...
- sudo/crontab (`272` §0): Cron mCell stored-in File:/var/spool/cron/crontabs/$(id -un); `resolve()`
