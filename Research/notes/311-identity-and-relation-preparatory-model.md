...
measurements; it never decodes a name. No single author knows the whole chain from a tool's
...
The model: every identity-bearing thing is an entity of a KIND. A kind's entities have an
AUTHORED naming inside a namespace (routing), and, where the kind's owner supplies a read, a
MEASURED naming inside another namespace (the identity chain). Identity is the measured
name scoped recursively until a ROOT, a declared global namespace, or a ROUTE, the last
resort. State has PLACEMENTS, many-valued, which feed collisions only. Two things compare
by walking their chains to the first divergence inside a shared scope and asking that
scope's naming what inequality licenses; never diverging is sameness; different roots are
unrelated. Every positive answer rests on an explicitly typed, long-named, absent-by-default
warrant owned by one author; silence is unknown. Transitions are of three species, each
perishing a different fact. The context slot survives as an ADDRESS and a witness key, never
as an identity input.
...
It reaches referents only through names, tokens, and witnesses, and every identity question
is ultimately "do these two names reach one referent". Referents are what GOTCHAS are about.
...
the authored naming discipline (§2.1), the read and measured naming if any (§2.2), the
placements (§2.3), the effect entailment (§2.4), the observer-dependence (§2.6), and the
decomposition of its names if it serves as a namespace (§2.8). A kind's owner speaks only
about the kind's own relations to its immediate neighbours; the engine composes. Kinds are
the only identity-bearing category; there is no separate entity type, aspect type, or
context type in the model (§1.8, §1.9).

### §1.3 name; the two namings
...
- an AUTHORED naming: the strings tool authors and books use (`red/7`, `/etc/nginx.conf`,
  `nginx`, `beta`), interpreted in the kind's authored namespace (§2.1);
- a MEASURED naming: the strings the kind's own identity read returns (an inode number, a
  filesystem identifier, a canonical package name, a boot identifier), interpreted in the
  kind's measured namespace (§2.2).

When a kind has no read, the measured naming IS the authored naming. The distinction exists
because the authored naming is what people write and is usually ambiguous (aliases, relative
names), while the measured naming is what the world answers and is where the dangerous
warrants can honestly be placed.
...
names are interpreted: a database file for account keys, a directory for the next path
component, a mount table for paths, a filesystem for inodes, a resolver-plus-vantage for
hostnames, a process table for pids. "Scope" is the same object seen from the name: the
scope of `red/7` is a particular database. A ROOT is a kind that declares its measured names
need no scope, which is the claim that they are globally comparable. Rootness is a dangerous,
long-named claim (§2.2); nothing is a root by default. `GOTCHAS: identity-tokens-have-
clone-horizons` is the standing witness against casual rootness.

### §1.5 token and its two warrants

A token is a measured name: bytes the kind's read returned, compared for equality only,
never decoded (`inv-referent-agnostic`). A token is always scoped (§1.4). Any naming,
authored or measured, has two independent properties, and each is a separately declared,
absent-by-default warrant:

- ONE-NAME-ONE-REFERENT: within one scope instance, equal names reach one referent. Licenses
...
- ONE-REFERENT-ONE-NAME: within one scope instance, one referent has one name. Licenses
...
The one non-unknown default in the model, flagged for the human's eye: for an AUTHORED
naming, equal names inside ONE scope instance across an UNWALLED span are taken to reach one
...
function opts out by declaring set-valued lookup. Everywhere else, including every measured
naming compared across scope instances and every root, both warrants are absent until typed.

### §1.6 resolution and its traversal

A resolution is the fact that authored name N, interpreted in namespace instance S, at
...
a service name). The namespace kind declares how its names decompose into traversals (§2.8).
A resolution perishes under ordinary effective-world reach from any mutator whose footprint
touches any traversal member. Its target object is NOT its backing: a name can stop reaching
an object without the object changing (`renaming-a-parent-moves-every-child-name`,
`a-path-is-not-a-referent`), and an object can change without its name changing.

### §1.7 chain; witness; witness set

A chain is the recursive identity of an entity: its measured name, scoped in its measured
namespace instance, whose identity is itself a chain, terminating at a root, at a route
(§1.9), or at unknown. A chain is one WITNESS to identity. A subject may carry several
witnesses with different generators: its enclosure chain, a provider-supplied identifier, a
correspondence asserted by a transition owner (§2.5). Witnesses combine by coherence
(§3.2), never by priority. Equality composes transitively across witnesses; separation is
decided at one divergence and is never chained.
...
another kind's authored naming (§2.7) but has its own read, measured namespace, placements,
and observer-dependence. `enabled` and `active` are two kinds sharing the unit name; one is
measured in a symlink's existence and the other in pid 1's memory, and their chains differ,
which is exactly why one survives a reboot and the other does not, with no declaration about
reboots by anyone. Three independent findings forced this fold (§6.2): one enclosure per
identity-bearing thing, per-aspect placement, per-aspect observer-dependence. The
whole-entity claim (`disturbs Service:nginx`) reaches its aspect-kinds by the owner's effect
entailment (§2.4), and absent that entailment collides with them as unrelated, which is the
safe bottom. The selector position in the coordinate and the selector dialect (`277` §3,
`30J`) have no counterpart in this model.

### §1.9 world; route; placeholder; witness at standup

A world is the ADDRESS a probe reached a referent from: the entry chain, expressed as a
finite map from namespace kinds to the instances that wrappers lent (§3.4), plus a vantage.
It is not part of any entity's identity. It has three jobs: it says where a read executes;
it supplies the ambient namespace instance for names whose kind is named-in a lent namespace;
and, for a kind with neither read nor namespace, it is the ROUTE, the last-resort scope
under which two same-spelled names in one unwalled span are one placeholder and anything
across worlds is unknown. Chains whose tokens are not yet measured are placeholders keyed by
(name, ambient instances, entry chain); the probe standup binds them; the apply standup
re-reads them through the same entry and compares; mismatch is integrity, never a verdict
input (`26Ob:cor-standup-witness-licenses-bare-line-elision`).

### §1.10 site; claim species
...
- a CORRESPONDENCE: name X in namespace instance A is name Y in namespace instance B, by the
  owner of the transition between A and B (§2.5);
- the kind-level DECLARATIONS of §2: namings, warrants, placements, decomposition,
...
### §2.1 named-in (authored naming; routing)

Kind K's authored names are interpreted in an instance of kind N. Declared by K's owner,
once per kind. The INSTANCE is supplied one of three ways, and the supply mode is part of
the declaration: FIXED (K's owner names it: the dpkg status file), SITE (the tool author's
bind fills it from argv: `--db`), AMBIENT (the entry chain's lent instance for N: the current
mount table for paths). Consumer: resolution (§1.6). Default: absent means K's names have no
...
### §2.2 the read; measured naming; enclosure; sole-route; rootness

K's identity read, declared and executed by K's owner in the world the name resolved in,
maps an authored name to a measured name scoped in an instance of kind M. M is K's measured
namespace; the M-instance is K's ENCLOSURE, the thing whose identity determines K's. The
read returns a scoped token, and the scope's identity is recursive (§1.7). Where no read
exists, M = N and the measured name is the authored name. Consumer: identity (§3.1).
Defaults: no read means identity falls to the authored naming under its own defaults.
Warrants, all absent by default and all declared by K's owner about the measured naming:

- one-name-one-referent and one-referent-one-name (§1.5), governing what token equality and
  inequality license at K's level of a chain;
- SOLE-ROUTE: K's referents are reachable only through their enclosure instance, so disjoint
  enclosures imply disjoint K-referents. Required for a divergence at the enclosure level to
  yield DISJOINT for K. Fails for replicated rows, for files reachable through several
  directories (a hardlink), for a record whose current state spans several files
  (`a-store-is-not-one-inode`);
- ROOTNESS: K declares no M, and thereby claims its measured names are globally comparable.
...
distinct from enclosure: enclosure is at most one and answers identity; placement is many
and answers interference. Two cells with different enclosures can share a placement and
therefore collide without being the same (`the-subject-includes-the-observer`: two observers'
...
definition and is the sole licensor of sparing across UNRELATED chains (§3.2). Declared by
...
consequence that no chain expresses.

### §2.5 corresponds (declared sameness across a transition)

Authored name X in namespace instance A denotes the same referent as name Y in namespace
instance B. Declared by the owner of the TRANSITION between A and B, which is neither
name's kind owner: the container manager knows guest pid 1 is host pid 4821
...
SAME witness (§1.7), vouch-tier, attributed to the transition author. Default: absent, so
names across a transition compare unknown unless a read binds tokens on both sides.
Danger: a wrong correspondence is a wrong SAME. This generalizes the mapped lend of `273`
and is the model's only declared sameness generator besides token equality.
...
O-instance. Consumer: the SAME consumer, as a qualifier on the subject. This is the surviving
half of the old invariance line (`271:rul-invariance-speech-act`): its store half is measured
away by §2.2, its observer half cannot be measured by any identity read because the object
is the same and the answer differs (`the-subject-includes-the-observer`), and it must remain
speech. Measurement in the denoted context (`plans/27C`) stays the default lane; carrying a
fact across an O-shift requires this declaration, and the human's hoped-for static
no-transit path costs exactly one such line per kind in the stdlib.

### §2.7 named-like (aspect-kinds)

Aspect-kind A borrows kind K's authored naming: A's entities are addressed by K's names in
K's namespace, and A has its own read, enclosure, placements, and observer-dependence.
Declared by A's owner, who is normally K's owner. Consumer: resolution of A's names via K's
§2.1. The whole-entity relationship between K:x and A:x is carried by K's `reaches` (§2.4);
without it they are unrelated and collide, which is safe.

### §2.8 decomposes-into (traversal structure)

A namespace kind declares that its names decompose into an ordered chain of its own
entities, each a routing entity the lookup crosses: a path into directory entries, a
hostname into resolver steps from a vantage, a dotted unit name into its instance table. The
engine derives the decomposition from name SYNTAX, which is language, plus this declaration
(`30T` §5's syntax-versus-semantics line, unchanged). Consumer: resolution backings (§1.6)
and therefore perishing (§3.3). Default: absent means the traversal is the namespace
instance as a whole, so any touch on the instance perishes every resolution through it, the
coarse and safe floor. This is what replaces authored region predicates: containment is
chain membership, and a mutator that touches a directory needs to know nothing about files
(`renaming-a-parent-moves-every-child-name`, `namespace-composition-is-not-concatenation`).

### §2.9 composite kinds (roles)

A subject whose value depends on several inputs IN ROLES (a base and an overlay; a primary
and its replica set) is an entity of a composite kind minted by the author who knows the
roles, normally the tool author, and that kind's identity is defined by its owner from its
named parts. There is no set-of-enclosures anywhere in the model: enclosure is at most one
(§2.2), and plurality of inputs is a kind with structure (`composite-identity-is-structure-
not-a-bag`). Placement of a composite is the union of its parts' placements.

### §2.10 relation table

| relation | arity | declared by | default | consumer | danger |
|---|---|---|---|---|---|
| named-in | one per kind, instance per name | kind owner (instance: fixed / site / ambient) | no namespace ⇒ route only | resolution | one-referent-one-name (disjoint from inequality) |
| read → measured naming | one per kind | kind owner | none ⇒ measured = authored | identity | one-name-one-referent · one-referent-one-name · sole-route · rootness |
...
| corresponds | per transition pair | transition owner | unknown | SAME witness | a wrong correspondence |
...
| decomposes-into | per namespace kind | namespace owner | whole-instance traversal | perishing | none (finer is value, coarse is safe) |
| lends (+ sentinel) | per wrapper, per namespace kind | wrapper owner | ⊤ ⇒ walls | ambient instance supply | a wrong lend measures the wrong world |
| composite kind | per composite | the author holding the roles | n/a | identity | as any kind |

## §3 — the model: composition (what the engine derives, and from what)

### §3.1 identity of an entity

identity(e of kind K) is the measured name of e (via K's read run in e's world, or the
authored name if K has no read), scoped in identity(enclosure instance), recursively, until
a root, a route, or an unknown link. Each level carries the warrants K's owner declared for
that naming. A composite kind's identity is its owner's function of its parts' identities.
An entity of an observer-dependent kind carries the O-instance as part of its subject.
Nothing about identity consults the world map except to know where to run reads and which
ambient instances to bind.

### §3.2 compare, in one chokepoint
...
`--risk-faultless-skips`; unknown and unrelated → the safe bottoms). For two chains:
...
- at the first level where the two names differ inside a shared scope S: DISJOINT iff S's
...
- if no level differs: SAME iff every level's equality is warranted (definitional for authored
  namings within one instance and unwalled span; one-name-one-referent for measured namings
  and roots); else UNKNOWN.
- any unknown link on either chain: UNKNOWN.

For witness sets: a warranted SAME and a warranted DISJOINT on one pair is a contradiction,
refuse both and attribute both authors; otherwise the strongest warranted answer stands.
SAME is transitive across witnesses; DISJOINT is never chained. Universal meet over backing
sets is unchanged (`set-lifting-universal-meet`): sparing needs every footprint-by-backing
pair disjoint. Cross-kind comparison needs no special rule: two chains either share a scope
at some level or they do not.
...
- A ROUTING mutation (a mount, a symlink replacement, a rename, a user added, a hostname
  ...
  entity perishes; every chain built on that resolution reads unknown below the line;
  dependent SAME conclusions lose authority and dependent elisions demote to guards;
...
- A STATE mutation writes cells through placements: ordinary kill-reach, unchanged. A first
  write can also change a measured name (`identity-tokens-perish-on-write-not-only-on-
  rename`), so a state mutation whose footprint touches an enclosure instance perishes the
  tokens scoped in it.
- A LIFECYCLE mutation (a reboot, a re-provision) disturbs a root-adjacent entity (a boot, a
  tenure); every measured name scoped in it names a new referent afterward; cells whose
  chains pass through it are new and unmeasured; cells whose chains do not are untouched.
  What earlier designs declared as "keyed by Boot" or "invariant across Boot" is the shape
  of the chain, not a declaration (`plans/30W` §7's patch day, derived).

In all three the engine withdraws authority; it never computes the successor identity.

### §3.4 entry: wrappers, lends, sentinel, correspondences

A wrapper's entry lends NAMESPACE INSTANCES for the namespace kinds it perturbs and nothing
else; the lent instances become the ambient supply for names whose kinds are named-in those
...
transitively through their chains with no speech from anyone (`not-every-transit-changes-
the-referent`). A wrapper may additionally declare correspondences across the namespaces it
lends (§2.5). Entry forms, siting vouches, the escalation dial, and measure-in-context remain
`plans/27C`'s; only the fallback lane changes shape (§3.6).

### §3.5 the committee law, satisfied
...
owner's enclosure declaration and the database owner's read, each speaking about their own
kind (`28M:rul-composite-meets-toward-guard-run`). A withholding composite (a mount perishing
an account's resolution) names nobody and needs nobody's consent. Attribution: every
survival names the sole-route and one-referent-one-name lines it rested on; every SAME
names the reads and correspondences; every perished conclusion names the footprint that
perished it.

### §3.6 what this model dissolves, and what it keeps
...
an authored region predicate (containment is chain membership); `resolve` as a member
...
coordinate in another kind, and identity is a chain of scoped names so no store-level
collapse follows); `279f` §3's refused transport chain (re-opened as chains of warranted
reads, not as backing completeness); `271:rul-invariance-speech-act` (re-read: textual
derivation never licenses; an authored read under a typed warrant does; the observer half
stays speech).
...
## §5 — UX, gradual enhancement, and teaching

*(deliberately empty; to be filled backwards from §1–§3)*

## §6 — the forcing functions

### §6.1 the slugged GOTCHAS and what each forces in the model
...
- `a-host-is-not-a-partition` — the world is an address, never an identity input; identity is
  a chain through the file's measured namespace (§1.9, §2.2).
- `same-name-different-referent-per-viewpoint` — the authored namespace instance can be
  ambient and lent; the chain differs per instance (§2.1, §3.4).
- `not-every-transit-changes-the-referent` — leaf kinds inherit through chains; a transit
  that lends no namespace on a chain leaves it untouched (§3.4).
...
- `distinct-names-alias-within-a-kind` — one-referent-one-name is absent by default; the
  read supplies the measured naming (§1.5, §2.2).
- `containment-by-path-prefix-lies` — the authored File naming is not sole-route; containment
  is chain membership on the measured side (§2.2, §2.8).
...
- `renaming-a-parent-moves-every-child-name` — decomposition into traversal; perishing by
  chain membership (§2.8, §3.3).
- `a-name-resolves-from-a-vantage` — the vantage is part of the world address and of a
  hostname's traversal (§1.9, §2.8).
- `resolution-is-set-valued` — the one definitional default has an owner opt-out; a
  set-valued lookup yields unknown from equal names (§1.5).
- `identity-tokens-have-clone-horizons` — rootness is a dangerous claim; one-name-one-
  referent is absent by default on measured namings (§1.4, §2.2).
...
- `a-store-is-not-one-inode` — placement is many-valued and distinct from enclosure; sole-
  route is not implied by "lives in" (§2.2, §2.3).
...
- `composite-identity-is-structure-not-a-bag` — at most one enclosure; roles are a composite
  kind minted by the author who holds them (§2.9).
...
- `identity-tokens-perish-on-write-not-only-on-rename` — a state mutation on an enclosure
  perishes tokens scoped in it (§3.3).

### §6.2 conceptual forcing functions: the dead ends and what killed each
...
- IDENTITY AS A PER-KIND TABLE AGAINST AXES (the trichotomy invariant/keyed/⊤ per
  ...
  of what its name denotes, not of the kind the name is written in.
- WORLD AS PART OF THE FACT KEY, argued as the root cause of the above. Killed by the review:
  a qualified key is an address of a question and asserts no partition; the dangerous
  inference lived in the meet's generators, not in the key's existence. Surviving form: the
  world is an address and a witness key (§1.9). This dissolved the A-versus-B binary of
  `311h`.
- STORE SETS, unioned and compared as bags (`311d` A1/A4; `311f`'s single `in:` for
  multi-store subjects). Killed by base-and-overlay: same set, different roles, different
  answers. Also killed by the reflexivity defect of pairwise set equality. Surviving form:
  one enclosure, composite kinds for roles (§2.9).
- "STORED-IN" AS ONE RELATION conflating routing and containment (`311d`; and the current
  ...
  disjointness is not referent disjointness. Surviving form: named-in, enclosure with
...
- TERMINAL TOKENS (`311f`'s `Measured(File, fsid:inode)`). Killed by NFS and by the scope
  question: an inode is a name in a filesystem, a filesystem identifier is a name in whatever
...
- AUTHORED REGION PREDICATES (`kind__disjoint`, `30W:rul-disjoint-is-an-rc-predicate`; the
  ...
  says nothing about whether changing one retargets a name for the other; and by the human's
  granule observation that the natural description unit is the atomically-disturbable one.
  Surviving form: decomposition into traversal; containment as chain membership (§2.8).
...
- MEASUREMENT MAKES DECLARATIONS REDUNDANT (`311h`'s framing). Killed by `311i` §0: a read
  establishes a token, not its scope, subject, warrant, applicability, or sufficiency.
...
- THE ASPECT AS A THIRD COORDINATE POSITION. Killed by three independent pushes (one
  enclosure, per-aspect placement, per-aspect observer-dependence). Surviving form: aspect-
  kinds (§1.8, §2.7). Recorded as forced, not chosen for tidiness.

## §7 — open questions carried into the next sitting (not rulings)

- `open-definitional-equality-default` — §1.5's one non-unknown default (equal authored names
  ...
  ever transports even within one world without a read.
...
- `open-witness-algebra-formalization` — §3.2's witness meet (coherence, transitivity of
  SAME only) is stated, not specified; the per-property meet direction registry `28M` §8
...
- `open-observer-namespace-inventory` — which namespace kinds parameterize VALUES (User) as
  opposed to only routing names; the model treats it per (kind, O) but the stdlib will want a
  short list.
- `open-context-slot-build-shape` — `plans/30W` item 1 builds the slot as a product over
  index-kinds; under this model the slot is an address and witness key only, and the
  trichotomy meet must not be built; the as-built audit the human deferred decides how much
  of the existing slot is reusable.
