...
> as a referent-agnostic example of an index or a transition.
>
...
mPrimaryKey inside another store (identity). Identity is the mPrimaryKey scoped
recursively, a mFullyQualifiedKey, until a mRoot, a declared global store, or a mRoute,
the last resort. State has mPlacements, many-valued, which feed collisions only. Two things
`compare()` by walking their mFullyQualifiedKeys to the first divergence inside a shared store
and asking that store's lookup what inequality licenses; never diverging is sameness;
...
the mNaturalKey lookup (§2.1), the `resolve()` and mPrimaryKey if any (§2.2), the mPlacements
...
key is relative to an index, and a bare string means nothing. Each mSort has:

- a mNaturalKey: the strings tool authors and books use (`red/7`, `/etc/nginx.conf`,
  `nginx`, `beta`), interpreted in the mSort's index (§2.1);
- a mPrimaryKey: the strings the mSort's own `resolve()` returns (an inode number, a
  filesystem identifier, a canonical package name, a boot identifier), interpreted in the
  mSort's store (§2.2).

When an mSort has no `resolve()`, the mPrimaryKey IS the mNaturalKey. The distinction exists because
...
### § 1.4-index-store-root

An index is an mKey of some mSort, used as the space in which another mSort's
...
hostnames, a process table for pids. An mSort's mNaturalKey is looked up in one such mKey (its
index) and its mPrimaryKey is scoped in another (its store); they coincide when the
mSort has no `resolve()`. A mRoot is an mSort that declares its mPrimaryKeys need no store, which is the
...
A mToken is a mPrimaryKey value: bytes the mSort's `resolve()` returned, compared for equality only,
never decoded (`inv-referent-agnostic`). A mToken is always scoped (§1.4). Any lookup,
natural or primary, has two independent properties, and each is a separately declared,
absent-by-default warrant:

- :guarantees-unique-referent: within one index or store, equal mKeys reach one mReferent; the
...
- :guarantees-unique-name: within one index or store, one mReferent has one mKey. Licenses
...
Both are properties of the LOOKUP between K's mKeys and mReferents inside any one
index or store, not of a single mKey and not of that index or store: declared once per mSort by
the owner of the mKeys, holding for every index or store they are looked up in. Neither holds by
...
A mResolution is the fact that mNaturalKey N, interpreted in index S, at
...
A mFullyQualifiedKey is the recursive identity of an mKey: its mPrimaryKey, scoped in its
store, whose identity is itself a mFullyQualifiedKey, terminating at a mRoot, at
...
another mSort's mNaturalKey lookup (§2.7) but has its own `resolve()`, store, mPlacements,
...
store per identity-bearing thing, per-aspect mPlacement, per-aspect
...
finite map from mSorts to the indexes that wrappers lent (§3.4).
It is not part of any mKey's identity. It has three jobs: it says where a `resolve()`
executes; it supplies the ambient index for mKeys whose mSort is :named-in a lent
index; and, for an mSort with neither `resolve()` nor `:named-in`, it is the mRoute, the last-resort
store. For transit-free local execution the engine itself vouches the mRoute: two same-spelled
...
mPlaceholders keyed by (mKey, ambient indexes, mEntryChain); the probe standup binds them;
...
- a mCorrespondence: mKey X in index A is mKey Y in index B, by the
...
### § 2.1-named-in (the natural key's index; routing)

mSort K's mNaturalKeys are interpreted in an index of mSort N. Declared by K's owner, once
...
fills it from argv: `--db`), AMBIENT (the mEntryChain's lent index for N: the current
...
claim that lookup in N is a function. This relation never licenses disjointness across indexes by itself:
index-disjointness is not referent-disjointness (`a-host-is-not-a-partition`,
`address-inequality-is-not-referent-inequality`, `containment-by-path-prefix-lies`).

### § 2.2-identified-in (the `resolve()`, the store, sole-route, rootness)

> ER modeling's identifying relationship: a weak mKey's mPrimaryKey includes its
...
maps a mNaturalKey to a mPrimaryKey scoped in an mKey of mSort M. That mKey is K's
store, the thing whose identity determines K's. The `resolve()` returns a scoped mToken,
and the store's identity is recursive (§1.7). Where no `resolve()` exists, M = N and the mPrimaryKey
...
- :sole-route: K's mReferents are reachable only through their store, so disjoint
  stores imply disjoint K-referents. Required for a divergence at the store level
...
- :rootness: K declares no M, and thereby claims its mPrimaryKeys are globally comparable.
  ...
  instances of its would-be store must be scoped in something smaller or left un-warranted.
...
distinct from the store: the store is at most one and answers
identity; mPlacement is many and answers interference. Two mCells with different stores
...
> this is the other case, same-mSort across two indexes, asserted by the one
> author who owns the transition between them (the NAT table, the pid-namespace map).

mNaturalKey X in index A denotes the same mReferent as mKey Y in index
...
mAspectSort A borrows mSort K's mNaturalKey lookup: A's mKeys are addressed by K's mKeys in
K's index, and A has its own `resolve()`, store, mPlacements, and
...
### § 2.8-hierarchical-index (versus flat; traversal structure)
...
index as a whole, and any touch on the index perishes every mResolution
...
stores anywhere in the model: the store is at most one (§2.2), and plurality of
...
|---|---|---|---|---|---|
| :named-in | one per mSort, index per mKey | mSort owner (index: fixed / site / ambient) | no index ⇒ mRoute only | mResolution | :guarantees-unique-referent (same from equality) · :guarantees-unique-name (disjoint from inequality) |
| :identified-in (the `resolve()`) | one per mSort | mSort owner | none ⇒ mPrimaryKey = mNaturalKey | identity | :guarantees-unique-referent · :guarantees-unique-name · :sole-route · :rootness |
...
| :hierarchical | per mSort | mSort owner | flat ⇒ whole-index mTraversal | perishing | none (finer is value, coarse is safe) |
| :lends (+ sentinel) | per wrapper, per mSort | wrapper owner | ⊤ ⇒ walls | ambient index supply | a wrong lend measures the wrong mVantage |
...
identity(e of mSort K) is the mPrimaryKey of e (via K's `resolve()` run from e's mVantage, or the
mNaturalKey if K has no `resolve()`), scoped in identity(store), recursively,
until a mRoot, a mRoute, or an unknown link. Each level carries the warrants K's owner declared
for that lookup. A mCompositeSort's identity is its owner's function of its parts'
...
and which ambient indexes to bind.
...
> the pair at all (no shared store, no mCorrespondence, no finished definition). It becomes
...
- at the first level where the two mKeys differ inside a shared store S: DISJOINT iff S's
  lookup for that mSort carries :guarantees-unique-name and every level above it carries
...
- if no level differs: SAME iff every level's equality is warranted (:guarantees-unique-
  mReferent on that level's lookup, or the engine's own transit-free local-mRoute claim of §1.9
...
mFullyQualifiedKeys either share a store at some level or they do not. A shared store
with equal mPrimaryKeys IS the cross-mSort SAME generator: two mSorts whose owners
identify into one store name one mReferent set through two mNaturalKeys, one thing in two
tables joined on a shared mKey, and this is the one place the corpus's "cross-mSort same does
not exist" is superseded. Genuinely different mPrimaryKeys for one mReferent (an NFS
filehandle and the server's inode; a machine-id and a cloud instance-id) are two mDerivations
for one mTopic, reconciled by §1.7's coherence, never a second mKey inside one store.
...
- A ROUTING mutation (a mount, a symlink replacement, a rename, a user added, a hostname
  ...
  deletion, and rename of an mKey are routing writes to its index entry (its existence
...
- A STATE mutation writes mCells through mPlacements: ordinary kill-reach, unchanged. A first
  ...
  whose footprint touches an store perishes the mTokens scoped in it.
- A LIFECYCLE mutation (a reboot, a re-provision) disturbs a mRoot-adjacent mKey (a boot, a
  tenure); every mPrimaryKey scoped in it names a new mReferent afterward; mCells whose
...
> A lend is dynamic binding: the wrapper rebinds an index parameter for the guest's
...
else; the lent indexes become the ambient supply for mKeys whose mSorts are :named-in those
mSorts. mSorts not lent inherit the caller's index only after the
...
mCorrespondences across the indexes it :lends (§2.5). Entry forms, siting vouches, the
escalation dial, and measure-in-context remain `plans/27C`'s; only the fallback lane changes
shape (§3.6).

### § 3.5-committee-law-satisfied

Every positive step is one author's line: a lookup and its warrants, a `resolve()`, a :sole-route
...
argv blindness (the mSite fills the index).
...
- `a-host-is-not-a-partition` — the mVantage is an address, never an identity input;
  identity is a mFullyQualifiedKey through the file's store (§1.9, §2.2).
- `same-name-different-referent-per-viewpoint` — the index can be
  ambient and lent; the mFullyQualifiedKey differs per index (§2.1, §3.4).
- `not-every-transit-changes-the-referent` — leaf mSorts inherit through their mKeys; a
  transit that lends no index on a mKey leaves it untouched (§3.4).
- `address-inequality-is-not-referent-inequality` — :named-in licenses no disjointness across
  indexes; :sole-route is a separate, dangerous flag (§2.1, §2.2).
...
- `containment-by-path-prefix-lies` — the File mNaturalKey's index is not :sole-route;
  containment is mTraversal membership on the mPrimaryKey side (§2.2, §2.8).
- `namespace-composition-is-not-concatenation` — indexes are mKeys with
...
- `resolution-is-set-valued` — :guarantees-unique-referent is the owner's opt-in per lookup; a
...
- `identity-tokens-have-clone-horizons` — :rootness is a dangerous claim; :guarantees-unique-
  mReferent is absent by default on mPrimaryKeys (§1.4, §2.2).
...
- `a-store-is-not-one-inode` — mPlacement is many-valued and distinct from the store
...
- `composite-identity-is-structure-not-a-bag` — at most one store; roles are a
...
- `identity-tokens-perish-on-write-not-only-on-rename` — a state mutation on an store
  perishes the mTokens scoped in it (§3.3).
- `a-recreated-name-is-a-new-referent` — creation and deletion are routing writes to the
  index entry; a stable mKey over a recreate is a perished mResolution (§3.3).
- `recycled-keys-outrun-the-unwalled-span` — no lookup is a function by default; a
  index that reissues mKeys never earns :guarantees-unique-referent, and the `witness()` cannot
...
- STORE SETS, unioned and compared as bags (`311d` A1/A4; `311f`'s single `in:` for
  ...
  one store, mCompositeSorts for roles (§2.9).
- "STORED-IN" AS ONE RELATION conflating routing and containment (`311d`; and the current
  design's keyed-by versus stored-in). Killed by hardlinks, bind mounts, NFS: index
...
- TERMINAL TOKENS (`311f`'s `Measured(File, fsid:inode)`). Killed by NFS and by the store
...
- ONE GRADE ON A READ (`311f` B3's transparent/identifying). Killed by clones: equal machine-
  ...
  form: two independent warrants per lookup (§1.5).
...
- SHARED ANCESTORS AS COLLISIONS (`311f` B2's cross-mSort rule). Killed by the package status
  file and the unit file sharing a filesystem. Surviving form: ancestors are stores;
...
- A DEFINITIONAL EQUAL-KEYS DEFAULT ("equal mNaturalKeys in one index across an unwalled
  ...
  Surviving form: the engine vouches only the transit-free local mRoute; every index's
...
- MEASUREMENT MAKES DECLARATIONS REDUNDANT (`311h`'s framing). Killed by `311i` §0: a `resolve()`
  establishes a mToken, not its store, mTopic, warrant, applicability, or sufficiency.
...
- THE ASPECT AS A THIRD COORDINATE POSITION. Killed by three independent pushes (one
  store, per-aspect mPlacement, per-aspect :observer-dependence). Surviving form:
