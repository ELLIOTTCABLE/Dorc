...
### Checkpoint D2b (`97f536e1`; both gate legs green; `bless:dry` clean; zero `DORC_KNOWN_BROKEN`; the proof census 242 both · 8 shell-only · 174 in-process-only · 0 refused · 0 disagreeing; 427 trials)

Built: the census tier's inputs moved to the loom corpus (`region_artifacts.rs`, the munge-witness roster re-keyed to stems); the loop30 pin on a plain `Unscheduled{end-of-r31}` with its reason, its fixture comment descriptive and re-blessed; the four fidelity fixes — (a) the binary's `dorc lint` operational error renders through the same parts seat as the in-process render and now CARRIES `error[slug]` (a product-facing diagnostic change); (b) the in-process driver reads the stdout posture from the seam, never from the block's `> /dev/null` (`script-relative-load-dies-slashless` gained its `artifact-form-fallback` line); (c) the in-process book-reach partition resolves `$0`-relative `.` operands through the binary's own `ScriptSpellings` resolution; (d) the contested-helper diagnostic's field and span locus render through `why::relativize_for_display` (the third seat of the B1/B2a class); the four `dorc lint` looms and the `lint/tests` `X/cmd` case are ordinary sessions (`run_lint`, `LoomRun`, `E2eKind::Lint`, the `X/cmd` shape gone); the two `lint-real-*` dirs are the real-tools test's own fixture space and the discovery floor is looms-only; the fixture seat KEPT (one live user, `receipt_route.rs`, behind the `fixture_intake_is_unreachable_from_production` fence; the dead `admit_fixture_inner_records` deleted); `ApplyWriteStep::word()` the one home for the five words; `mise run test`'s doc run quiet (260 → 152 lines, the load-bearing compile-fail doctest intact); `yardstick` and its script DELETED (no live citer; every task is shell-free).

| item | ruling | the conductor's own mistake, named |
|---|---|---|
| 8a and 8b deferred at budget, with the scope finding that `xfail`'s tree walk needs `repo_root` (a fifth tenant) and that `oracle` carries an unused `internal-tooling` dependency | ACCEPT the clean stops; D2b′ (a small lane) takes both tenants, `repo_root`, and the stale dep. | the D2b brief was the arc's largest remit after the D-lane split had already doubled twice. |
| the fixture seat kept · the discovery floor looms-only · the two re-blesses (loop30, the relativized `load30-subshell`) · the incidental dead-helper and clippy cleanups | ACCEPT all (each on its own ruling). | — |
