...
- **concl-pivot-is-chartered** — `ack-pivot-must-support` (human-typed):
  ...
  scope-typing seam — which this round hit FOUR independent times (host×epoch
...
- **concl-inside-is-alive-narrow-and-cheap** — the two-conjunct discriminator
  (re-runs against mutable state ∧ host assists nothing) is corpus-native
...
3. sit-scope-ownership — line-said vs engine-derived execution scope (oracle-
   ...
   (four arrivals; host×epoch; split-books ruling standing meanwhile).
