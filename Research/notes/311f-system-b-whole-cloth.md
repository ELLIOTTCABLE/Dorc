...
kernel parameter, a running process, a machine. mCells are (referent, aspect); aspects are the
owner-minted selectors, unchanged.

NAME — how a tool addresses a referent FROM SOMEWHERE. A name is always relative to a
mNamespaceInstance: `red/7` relative to a database file; `/mnt/team/people.db` relative to a mount
...
mNamespaceKind (routing) — the thing a name is interpreted IN. Itself a referent of some kind
(a File; a MountNamespace; a Host-as-mVantage). mNamespaceKinds are what viewpoint transitions
perturb: `chroot` swaps the mount table; `ssh` swaps every ambient mNamespaceInstance for the far
host's; `sudo -u` swaps the identity user-relative names resolve against; `mount` and `ln -s`
and `mv` MUTATE a mNamespaceInstance in place for everyone after the line.
...
mNamespaceInstance-disjointness never implies referent-disjointness; but a row cannot live in two
database files, so store-disjointness does imply referent-disjointness. Which of the two a
mKind exhibits is the mKind owner's to say, and the current design (and System A) conflate them
under one "stored-in".
...
recursively, terminating at a mKind whose owner supplies a MEASURED mToken (a `resolve()` run
in the mWorld the name was resolved in), or at Top. The mNamespaceInstance a name was resolved THROUGH
...
`Route` is the honest floor for mKinds that declare neither store nor `resolve()`: their referents are
...
## B2. compare(R1, R2) — one recursion, four answers
    ...
    different kinds: compare the store chains; disjoint iff both chains are TOTAL and share
                     no same-or-unknown store pair; same never (cross-mKind co-reference stays
...
is provenance, and the `witness()`'s job is to confirm the mRoute still reaches that referent at
...
mKind owner, per mKind:
- `kind__identity()` — the ONE identity member. Input: a canonical name (or the store's
  handle for store-relative mKinds); runs in the denoted mWorld; prints one mToken line; rc is
  ...
  sentence. `resolve` is FOLDED into this: for a symbolic mKind the mToken is the canonical
  name (`dpkg-query -W -f '${Package}'`), graded identifying-or-injective; for a measurable
  mKind it is the measurement (`fsid:inode`). One member, one question: "what is this, as far
...
- `kind__located_in()` — per selector: `: lives-in KIND[:fixed-entity]` (the store; where a
  ...
  (the routing mNamespaceInstance, where names are ambient-relative). One line `: located-in KIND`
  when they coincide (Account: both the db File). Totality: `located nothing-else` per arm.
  Injectivity of the mKind's names within one store: `: names-are-distinct` (default absent ⇒
  unknown on name inequality; silence licenses nothing, no disclosed-weak floor).
- `kind__disjoint()` — region containment for path-like mKinds (unchanged ruling); used for
...
- binds carry BOTH the mEntity and, where argv supplies it, the store instance:
  `db : sm.dorc.File = "$dbpath"` then `acct : org.acct.Account = "$dir/$num" in "$db"`
  (STRAWMAN `in`: the bind's second clause names the store instance for a mKind whose
  `located_in` says the store is a File). Marks and disturbs emissions then carry the bound
  mEntity, whose Ref already includes its store; no per-line `stored-in` repetition.
- verdict marks, observe marks, `disturbs` emissions, `predicts` — unchanged.

Wrapper oracle:
- `cmd__lend_map()` lends ROUTING mNamespaceInstances for the mKinds it perturbs, and nothing
  else: `sudo -u X` lends the User mNamespaceInstance; `chroot D` lends MountNamespace :=
  Located(MountRoot, D, in: ambient); `ssh H` lends Host := Located(Host, H, in: mVantage) and
  RESETS every ambient mNamespaceInstance to the far mWorld's (the reset sentinel); `nice` lends
  nothing. Absent mKinds inherit after a `lends nothing-else` sentinel, else Top. The map is an
...
Mutator oracles:
- a routing mutator (`mount`, `ln -s`, `mv`, `useradd`, `hostname`) `disturbs` the mNamespaceInstance
  mCell (`sm.dorc.MountNamespace:"$ns"@table`, or a File region via `disjoint`). A mResolution
  (name → store instance, made under that mNamespaceInstance) is a fact whose backing is the mNamespaceInstance
...
- `located nothing-else` (mKind owner): "my referents live in these stores and no other".
...
- `names-are-distinct` (mKind owner): distinct canonical names in one store are distinct
  referents. Wrong for a path-like mKind = the synonym knife (`23M`'s dangerous mCell).
- `transparent` grade on a `resolve()` (mKind owner): unequal mTokens are different referents. Wrong
...
- `ssh beta acct --db /mnt/team/people.db ...`: ssh lends Host=beta and resets ambient
  mNamespaceInstances. `--db` is bound as a File in beta's MountNamespace; File's `identity` runs on
...
- `chroot /mnt apt-get install x`: MountNamespace lent; Package's store File
  ...
  "package mKind must not claim fs-view invariance" becomes a measurement, not a rule.
- `sudo -u alice crontab -l` vs root: Cron located-in File:/var/spool/cron/crontabs/$(id -un);
  the who-am-I ingredient resolves under the lent User mNamespaceInstance to two names, two inodes ⇒
...
- A mKind with nothing declared (the two-minute oracle, the auto-mCell): Route(E) at the
...
- The context slot as a component of a fact's mKey. mWorlds are ROUTES: an analysis-time
  environment of routing-mNamespaceInstances, consulted at mResolution, recorded as
...
- Substrate mTokens. mKinds.
- The name floor as a default. Injectivity is declared per mKind; silence ⇒ unknown.
...
- The one-sentence volume identity of `30W` §5. Replaced by File's `resolve()` plus the store
  chain; a Volume/Filesystem mKind's `resolve()` is where NFS fsid versus local UUID is decided, and
...
- Value regression for lazy mKind-minters: without `names-are-distinct`, same-mKind
...
- More authored `resolve()`: every store mKind in a chain needs an `identity` for the chain to
...
- A representational rewrite: facts carry Refs, not (coordinate, context). `30W` item 1's
  context-slot product is NOT built; something narrower (the ambient mNamespaceInstance environment)
  is built at the analysis seat instead.
- The routing/containment split is one more distinction for mKind owners to learn. It earns
  its place: conflating them is a wrong-disjoint on every path-like mKind.
