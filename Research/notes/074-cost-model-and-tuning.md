...
5. **A separate `.perf-info` artifact** → ceremony; prefer a one-line class, or infer the class from the predict body (static inspection: a `curl` in the check ⇒ `network`). Spike the inference.
