...
The pipx author describes a kind for installed apps whose natural key is the app name.
Named-in: the pipx store, a directory. Placement: that store and the shim directory. Both are
File entities, so the author names them. `/root/.local/pipx`? `/home/alice/.local/pipx`? Every
...
`311` § 2.1 supplies a namespace instance three ways: FIXED (the owner names it), SITE (the
tool author's bind fills it from argv), AMBIENT (the entry chain's lent instance for that
namespace kind). The pipx store is none of these. It is computed: `${PIPX_HOME:-$HOME/.local/pipx}`,
...
rebinds `PATH`. The engine's route claim must perish on a ρ write, not only on a mount-table
...
leverage. An omitted placement is the silent channel under the flag. A wrong `HOME` lend from
the sudo author mis-routes every `$HOME`-keyed store under every wrapped site everywhere: the
...
author claims nothing about the environment: the instance is unknown, the key is unknown, guard
or run. Monotone: every added half buys its sites and none removes a prior one.
...
- `lead-env-is-a-routing-namespace`: ρ routes command words (`PATH`), stores (`HOME`, `XDG_*`,
  ...
  severs for verdict bodies; the routing role is wider and belongs in 311's namespace inventory
  (`311:open-observer-namespace-inventory`).
- `lead-local-route-perishes-on-rho-writes`: the engine's own route claim for command words is
