...
S1, the floor: two targets, one NFS export, no roots declared anywhere.
...
Compare: two routes; § 1.9 says unknown. Collide; the guard stays; at apply it re-reads and
...
Same root kind; first divergence at the top; instance-ids carry guarantees-unique-name. § 3.2:
DISJOINT if "every level above it carries sole-route." Read root-ward, nothing is above:
...
§ 3.2: one side a route, the other not: UNSPOKEN. Sparing rides the footprint side's finished
...
S4, the SAME direction, the world of S1 plus the admin's own mount line:
...
Two derivations now exist for the pair: the key walk (unknown) and the correspondence (SAME,
...
- `obs-unspoken-is-kind-level-only` — § 3.2's route-versus-root and roots-differ clauses read
  ...
  world, which is what 30U's finished definition was written to spend; a route or a second root
  is a second world, and no finished definition speaks to it.
- `obs-partial-measurement-must-never-widen` — S3 against S2: a chain with fewer measured links
  compared no less decisively. A monotonicity law is owed: a derivation with an unmeasured or
  route-terminated link yields at most what it yields with the link measured. `16P` DP-8's
...
- `obs-sole-route-runs-leaf-ward` — § 2.2 defines sole-route as what lets a divergence at a
  scope level license disjointness beneath it; § 3.2's "every level above it", after "walk from
  the roots", reads the other way. State the orientation: every level between the divergence
  and the leaf.
- `obs-rootness-is-retroactive` — a true root anywhere up a chain converts every lazy scope
  beneath it, fleet-wide, from unknown to decisive. The blast radius is not the root's own
  referents; it is every kind that identifies into it. Countability: roots are stdlib-only and
...
- `obs-the-finished-definition-is-within-world` — "nothing else, in any vocabulary" is not "in
  any world"; the engine must not spend it across routes or roots; when it does, the Package
...
- `obs-the-filesystem-owner-must-know-network-filesystems` — `311` § 6.2's "the File owner
  ...
  server (an unknown link, through the vantage) or decline. A statfs fsid scoped in Boot is
  true and unsafe.
- `obs-cross-target-disjointness-is-the-admins-sole-route` — S6: "these targets share no store"
  is sole-route at the Target scope, the admin's seat under the posture option
...
- `obs-attribution-tests-the-rule` — for any fix: when the case fires, the why chain must name
  ...
  never one that could not (the Package author's within-world sentence).
