...
inline oracle dialect is STRIP-ONLY; the predict() IS the oracle; the st-2 check/probe split is
...
changes are in `spike/crates/oracle/src/predict/` — ADDITIVE (old forms still parse identically; the new
data is carried but not yet consumed by the lift/emitter), so the whole baseline stayed green.

- **R1a — period-named funcdefs.** `apt-get.predict() { … }` parses as an oracle check for provider
  `apt-get` (`check/parser.rs::at_check_funcdef`, new `PERIOD_CHECK_SUFFIX`). The period form's
  provider is the literal command word (keeps `-`); the legacy `apt_get__predict` form still works
...
- **R1c — strip pass** (`check.rs::strip_predict` + `collect_strip_edits`). Surgical byte-span edit
  over the funcdef source (needs new spans: `Check.span`, `Annotation.name_span`/`value_span`,
  `Mark.span`): rewrite the name to `<mangled>__predict`; identity `name : kind = value` → `name=value`;
...
16 new tests (12 dialect-parse in `predict::parser::dialect_tests`, 4 strip in `predict::strip_tests`).
...
23D §1 and R1a write the strip form as `name.predict()` → `name_predict()` (single underscore), but the
engine's `CHECK_SUFFIX` and the flagship golden both use `__predict` (double). I made the strip produce
`__predict` (the engine/golden convention wins — otherwise the reingested-name-collision case
...
- `dorc_plan::compile_probe(…, |kind, selector| idx.resolve_probe(…))` — R3 changes this closure's
  signature (it must source the *stripped predict body + the site's argv*, not a `(kind,selector)`
...
- `dorc_oracle::lift`, `KindIndex::resolve_probe`, inline oracle fixtures using
  `oracle_kind`/`oracle_probe`/`oracle_effect`/`__predict` (coverage/lib.rs ~L1134–1154, main.rs
...
but from the predict body's `case $verb` arms + trailing marks instead of `oracle_kind`/`oracle_effect`/
`oracle_probe`. Zero behavioural change to the analysis pipeline; only the authored surface changes.

The derivation (per-check, statically walking the `PredictSet` — no book argv, so no value-flow needed):
...
- **probe body** ← R3 (the stripped predict body); the per-selector distinction that `oracle_probe_*`
  encodes is reproduced by the predict body's per-verb arms (`systemctl__predict enable` runs is-enabled,
...
- **jc-dpkg-i (STOP):** `fixtures/package.oracle.sh` (and its unit test
  ...
  effect for provider `dpkg` with **no `dpkg__predict` function**. Under R2 (effects derive from check
  bodies), there is nothing to derive it from. Options: author a `dpkg.predict()` (feasible: `case $1
  in -i) …`), or drop the effect (breaks the pinned unit test). Deriving it "from the predict-body's
  argparse where the fixtures make it unambiguous" fails — there is no such predict body. Conservative
  mechanical option: author a `dpkg.predict()`; but that is *new oracle authoring*, not conversion, so
...
- **jc-fblessed (STOP — semantic):** the multi-selector F-BLESSED floor (`KindIndex::resolve_probe`
  ...
  the probe is the predict body, so the floor's basis evaporates: the engine cannot verify that the
...
`oracle_probe_*` body wrapped as `<kind>_<selector>__predict`.

Concrete change (all in `plan` + the `cli`/`analysis`/`hostsim` wiring):
- `ProbePredict.sh` becomes `strip_predict(oracle_src, check)`; the invocation becomes `<provider>__predict
  <site-argv…>` (via `render::probe::invocation`), not `<kind>_<selector>__predict '<entity>'`.
- Wrapper dedup keys per **provider** (per check fn), not per `(kind, selector)`. A multi-selector
  kind ships ONE wrapper invoked per verb (service: one `systemctl__predict`, invoked `enable`/`start`).
...
- +SURE the probe rc is preserved: `apt_get__predict install -y nginx` → `dpkg-query -W nginx` → rc-0
  iff installed, identical to the old `package_installed__predict 'nginx'`. The exec gates (dash -n,
...
crate. The signature change is unavoidable (the emitter must source the predict body + argv, a different
...
history), phrased to state the design truth (st-2 is spike-internal fiction; the stripped predict body
...
- **package (apt-get):** `apt-get.predict()`; keep the identity `pkg : package = "$1"`; add
...
- **seam (apt + yum):** each provider ships its own `.predict()` with its own probe (`dpkg-query` /
...
- **jc-strip-funcname** (§1) — strip emits `__predict` (engine convention) not `name_predict` (23D text).
