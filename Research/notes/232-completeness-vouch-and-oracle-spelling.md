...
- The inline `: Kind#effect_class` form is **not built**: `effect_class` appears nowhere in the worktree; the inline annotation is **identity-only** `name : reverse.dns.Kind = value` (`oracle/src/check/ast.rs:88-111`, `204:43`), carrying no channel/selector.
