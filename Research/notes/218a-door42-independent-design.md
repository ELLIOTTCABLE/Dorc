...
- **`set -u` hazard** (~SUSPECT, build-verify): the guard body executes under the *book's* shell options. A body that is probe-lane-clean can die under the book's `set -u` (an unset-parameter expansion error in dash is fatal even in errexit-exempt context, ~SUSPECT — must verify in-build). Conservative candidate: wrap the call `( set +u; package_installed__predict 'nginx' ) >/dev/null 2>&1 || …` (a subshell fork, ms-class, restores the probe lane's effective environment). Optimistic candidate: bare call + an oracle quality-bar rule (R2-class) "probe bodies must be set-u-clean". Build feedback decides (u-11); the strawman above shows the optimistic form.
...
```sh
package_installed__predict() { …dpkg-query…; }
set -e
package_installed__predict 'nginx' >/dev/null 2>&1 || apt-get install -y nginx
```
