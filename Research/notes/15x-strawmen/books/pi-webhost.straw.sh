#!/bin/sh
# ── book: bring up my pi as a webhost ──────────────  [STRAWMAN, for discussion]
# Scrappy, target-specific, low-reuse — the lazy-admin end. Pure POSIX (runs in
# dash); NO oracle verb-ladder, NO dot. Quality is deliberately uneven: some
# idempotency guards an admin writes anyway (the parts Dorc can lift), and some
# bare mutations with no guard at all (where Dorc would have to borrow a probe
# from an oracle to know whether to skip).
# Corpus idioms: case $(hostname) targeting, command -v || install [src-W4-13],
# systemctl enable --now [src-W4-13], [ ! -f ] config guard [src-W1-30],
# nginx -t before reload [src-W4-31/33], sentinel touch.
