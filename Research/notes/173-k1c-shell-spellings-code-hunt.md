...
> between commands. Feeds `plans/175` q1. Sources → `../sources.json`. AI-generated; confidence-marked.
>
...
- **k1c-handle-is-the-name (+SURE) — the coordination handle is the env-var NAME, convention-agreed across
  ...
  **precondition** on the shared handle: `[[ -z ${KUBECONFIG} ]] && die "KUBECONFIG needs to be set"` before
  ...
  handle (`plans/175` C2) — flat/conventional, not reverse-DNS, but the same idea: decentralized agreement
...
only; the KIND stays convention/author-declared** — the third converging confirmation of `plans/175`'s
...
## Open / next (→ plans/175)
- Fold k1c into `plans/175`: extend C2 (the handle) to *"reverse-DNS string OR a convention-named env-var,
