# 30D — The minispec review pair: adjudication

> Tier: conductor adjudication (Fable, 2026-08-16). Inputs: `30B` (Fable-class
> neutral) · `30C` (Sol adversarial, foreign lineage) — independent clean contexts,
...
- **The corpus is broken at its root, today**: `minispec/Minispec.lean:13` imports
  ...
  Lean tiers are opt-in — the exact composite `30B:fnd-earned-badges-are-unchecked-
  on-every-gate-that-runs` describes.
- `Minispec/Proofs/` holds only `README.lean` — consistent with `proved` never
  having been claimed; the text-grep trap (`30C` finding 4) is laid, not sprung.

## §2 — Credited on convergence (repair-lane verify-first items)

The full statements live in the reports; credited headline set:
`30B:fnd-lean-staging-never-removes-stale-files` (a SHARED cross-worktree build
...
`30B:fnd-battery-never-instantiates-its-own-law` (badges are evidence about the
...
where the laws bind `Flat::join` (`30C` finding 2 + `30B`'s render half) ·
...
(`30B` verified the three missed names by hand) · the promote generator remains
...
both production instantiations, `Flat<String>`/`Flat<Binding>` — `30B` checked);
...
- The repair surface SPLITS on `minispec/CLAUDE.md`'s own line: everything in
  `spike/verify/` is ordinary builder-space (`30B` §5's ranked list items 2–8,
...
its cache accreted. Instruments need their own instruments; the `30B` §5 item-3
