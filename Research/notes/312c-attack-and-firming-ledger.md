...
> human calls it stable; the concretization step (names, spellings, user-facing usage) is the
> `312` synthesis and comes after. Nothing here is ruled unless it cites a ruling by
...
> welds, `notes/311j`, and `notes/312a` § 0 (the phase's conduct as typed) outrank this.
> Successors append sections at the tail and edit nothing in the middle.
...
- Every keyed term stays tagged wherever it appears, grep-ability over grammar; a bare
  "sort", "key", or "scheme" never stands in. "Lookup" and "value" stay untagged.
- Edits are ahistorical: remove, never replace with a negation. A blockquote names a false
  friend in one line and does nothing else.
- Strawman conventions: a trailer sits on an executable line; an attribute takes exactly one
  quoted parameter (`: attr "a,b" attr ""`); attribute names are a few hyphenated words.
- The `311` series stays hyper-abstract (model, terms, laws); `312` is concretization, after
  311j is stable.

The arc (**[HUMAN]**): proposed resolution (done) → attack and refinement (this ledger) →
