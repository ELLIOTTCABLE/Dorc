(QUARANTINED priming-prompt for spike-3 / "take-3" — round 20. Paste the body below into a
fresh, in-worktree, Fable-class top-level session. Authored 2026-06-09 by the main-session agent
at human direction, from 192's prompts + the round-19 hand-forward (19F/19G/19H/19I) + the
first-party Fable prompting guidance (goals-and-invariants over procedure; brief steering over
enumeration; one gate instead of the old two-prompt series — Fable wants the full task up front).
Residual precondition (as of 06-09 evening — the round-19 merge and kSILO are in): commit
STALENESS-AUDIT.md before launching; the orientation below reads it, and a fresh worktree sees
...
contradiction now is worth more than a week of building on it. Wait for my go.

Then the take-3 material:

4. Research/plans/19H — what take-3 builds. Reference-quality; treat it as your near-charter.
5. Research/plans/19I — what you're graded against: the 43-case corpus as a behavior-acceptance
   measuring-stick, its four stand-in axes, and the cruft to strip.
...
8. The spike-2 code at Research/notes/quarantine-DO-NOT-READ/spike2/ — your starting workspace.
   The quarantine is readable for you and your subagents this round: it exists to keep unprimed
   sessions clean, and you are being primed deliberately.
...
- Seed the workspace: copy quarantine spike2/ to <worktree-root>/spike/, `mise trust` the
  relocated configs, verify green (`mise exec -- cargo test --workspace`; `sh e2e/run.sh`), then
  de-cruft per 19I §2 — strip the rc-injection mechanism and masking tests, the baked-verb
  fixtures; keep the exec-under-mocks gate verbatim; the standing xfail converts from pin to
  requirement.
- Build the input side 19H specifies: a real value-flow analysis — constant plus
  ...
  annotation) and shipped as the read-only probe body; and completion of the one-Observable
  unification where it's still half-done. Replace every stand-in 19I tags with the real
  mechanism. A case that passes because a fixture happened to feed the right value is not a pass.
...
- Open forks — decide deliberately, record the choice and its cost in a note, don't bake
  ...
  test front-end, and don't build the strip/transpile pass); and 19H §4's substrate question —
  whether the widened value-flow rides the existing worklist. A short prior-art check there is
  yours to judge; don't pre-pay for machinery the seams haven't demanded.
...
1. KNOBS we'll turn by choice throughout the future process — keep their decision-points alive
   in the code's shape; don't weld a pole in passing.
2. Decided things that are still inherently balances — the four-outcome priority lattice;
   fail-fast batching of unrelated errors vs warning-fatigue; degradation that is proportional
   to the user's omission and never an inch further.
3. Implementation-balances that were never anyone's "design decision" — where and when to mint a
   ...
   per-file complexity dial runs before degrade-to-⊤.
...
- Get better at dispatching as you proceed: continually refine your working model of
  ...
  expensive and will exhaust my tokens quickly; the balance is delicate. One unvalidated
  rule-of-thumb to test rather than trust: try Opus first, but avoid burning rounds on
  corrections-and-repairs — give up and graduate the task to Fable when Opus fails or bogs down.
  Where a refined heuristic earns it, write it down in your notes; future rounds inherit it.
