# 091 — specimen: stack's `get-stack.sh` — the RAL by hand, and the m×n we abdicate

> AI-generated literate specimen. The code below is reproduced VERBATIM
> (commit-pinned + byte-checked by inline-specimen.sh); annotate it ONLY by
> editing around the source — never retype a code line. Style: see specimens/090.

## Source & attribution
- Repo: commercialhaskell/stack — file `etc/scripts/get-stack.sh`
- Permalink (commit-pinned): https://github.com/commercialhaskell/stack/blob/7c519cb2b71ae91bc239248a3b9d2c5bb702be8e/etc/scripts/get-stack.sh
- Raw: https://raw.githubusercontent.com/commercialhaskell/stack/7c519cb2b71ae91bc239248a3b9d2c5bb702be8e/etc/scripts/get-stack.sh
- Commit: 7c519cb2b71ae91bc239248a3b9d2c5bb702be8e
- License: NOASSERTION
- Retrieved: 2026-06-03 via inline-specimen.sh (gh + curl); sha256 `cfc74001e126a0fff5dad961aec5b3bda43f29b1174d3538d1ee56248cdf7e2a`

---

## Why this specimen: the motivator for abdication

`stack`'s installer is the cleanest wild instance of the Puppet-RAL pattern (note 092 `f17`), and it earns the log as the *cautionary tale* that motivates why Dorc **abdicates** a whole class of knowledge instead of baking it in. Dispatch core to read for: `do_os()` (case on `$(uname)`) → `do_distro()` (case on `$DISTRO`, gathered by the `try_lsb || try_release || try_issue` fallback cascade in `distro_info()`) → each `do_<distro>_install` redefines a local `install_dependencies()` calling the right `{apt,dnf,yum,apk}_*` helper; `has_X` predicates and `get_isa`/`get_arch` canonicalise capability and arch (so `arm64 ≡ aarch64`). Everything past the dispatch is download/verify/usage boilerplate, kept verbatim but uncommented.

**The finding (`abdicate-cautionary`).** The cost of *not* abdicating is not one table but **m × n**: every alias/equivalence table (`ubuntu|linuxmint|elementary|neon|pop`, `arm64≡aarch64`, the three detection strategies) times *every kinded concept that exists*. Baking that in is TypeScript shipping a maintainer-arbitrated list of Every Thing as a built-in type. And the abdicated object is not "packages" — it is the higher-kinded relation *"these two oracles, by authors who don't know each other exist, both `provide X-widget`,"* including their subtle handling differences (is it `nginx_server` under brew? does debian ship without the default conf?), for an `X-widget` Dorc never knew was a concept. Any "we-can't-enumerate-the-universe" concept (system/language package managers, docker images, npm deps, …) collapses to this same shape.

**What we cannot abdicate — the meta-contract debt.** We abdicate the *content* (the universe of kinds and their providers) but we owe the *mechanism*: a plain-sh idiom by which an oracle-author declares, to other oracles that may never know it exists, "I provide kind-K," "K-here ≡ K-there," and "my K has these wrinkles." Designing that idiom is the real problem this specimen frames; the abdication is only free once the meta-contract exists.

**Three threads (recorded, not resolved):**
- *`$PATH` as availability-truth* (the human's live hypothesis): the common query "is nginx *runnable*" collapses to a `$PATH` check, installer-agnostic — a **bakeable** core fact — leaving only full state-management (install/version/conf) abdicated. First-classing `$PATH` may cleave "available?" (core) from "managed?" (abdicated).
- *the second abstraction axis* (gut-level): the `neon`-in-the-alias-list problem is the *apt-oracle author* not knowing `neon` exists; a real ecosystem needs an unrelated contributor to declare `neon ≡ ubuntu` to **all** `package`-oracles without editing any of them — genuine type-theory (instance/coherence) territory, flagged for the contract design.
- *os/arch are higher-kinded `≈ packages`*: likewise-abdicated state-enums; `distro_info()`'s ugly multi-strategy probe is exactly the gnarly, host-specific fact-gathering an oracle should *own as one grounded leaf*, not something Dorc bakes.

This is the **last abdicate-bucket specimen** worth baking — it motivates the abdication and names the meta-contract debt; subsequent specimen-hunting pivots to **bake-into-core** correctness patterns (the control/state-flow the analyzer must model itself, à la `specimens/090`'s transient-state). The verbatim source below is the evidence.

## The script (verbatim; commit-pinned above)
