...
2. `mktemp` paths + `trap '…umount…' ERR` establish transient state. `[bless?]` this is the named candidate — bless `trap … EXIT/ERR` as *the* canonical "this fact is transient, do not hoist its dependents into the resting-state probe" marker, and deliberately miss every ad-hoc mount/save spelling. That one mechanical rule is what lets the designer *refuse* to enumerate the mount zoo and push the work onto oracle authors (wrap transient state in `trap`, or be missed) — the library-bloat bound, TS-type-guard-style. (To be exact: this is a **contract** on *oracle-authored* code — the author *declares* the transient via the blessed `trap`, overriding the analyzer's would-be ambient hoist. Dorc does **not** *detect* transient-ness from a `trap` (Rice/W1 forbids deciding it; the trap-free do/undo is identical — see `093`). "Or be missed" means absence ⇒ the analyzer may wrongly hoist, so the burden is the author's; the `trap` adds a don't-hoist signal where present, never licenses hoisting where absent.)
...
**Meta**: even this careful, comment-rich author derives rich idempotency / freshness signal *for free* (the transient state it must *declare* — here via `trap` — a contract, not a free detection; see `093`), yet leaves dependency-presence (assumes `mkosi`, `guestfish`, `bindfs`, `proot`, `stap`, `jsonnet` exist — no `command -v` guards) and provider-equivalence entirely to convention. That is the right division: those are the abdicate column. The empirical shape is "bless the cheap structural facts, abdicate the open-ended equivalences."
