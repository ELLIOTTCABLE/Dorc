...
| an-entity-shape | flat (`package:nginx`) vs structured w/ selectors (`{installed, version, held}`) — re-keys every transfer + the substrate | precision/recency layer | — | 16Q dq-entity-algebra, 055§1B, 17N§4 | O |
...
| an-entity-coref | intra-script co-reference: do two opaque tokens denote the *same* entity (engine compares, never decodes). r24: within-kind aliasing CLOSED for resolver-bearing kinds (an-kind-resolver); CROSS-kind co-reference = the post-trial design seed (kind-fence precedes canonicalization; CanonicalCoord can't carry a target kind) | an-entity-uniqueness; an-reaching-ambient | — | 16P-T7, 055 dec-5, 24C§5A | B |
...
| an-named-kind | cross-oracle identity binds to a *named kind*, never a shared arg-token (token-equality collides across oracles). r24: + kind-owner canonicalization/closure (an-kind-resolver / an-kind-reach) | cross-oracle coherence | an-opaque-token | 099-C3/W4, 16P-T7, 24G | B |
...
| an-volatile | nondeterministic state (clock/`$RANDOM`/network) — distinct from transient; ⊤; canonicalize for any cache. Flag (2026-07-05): wall-clock-keyed verdicts (pkgindex "fresh", cert expiry) = first hermeticity test-cases for any kSTATE cache | kVOLATILES; memo hermeticity | an-ambient-vs-transient | 090§1, 076#10, 099-C4 | S |
...
| an-probe-shape | probe must capture the tool's *own* rc — `cmd \| grep -q` can't emit Unknown (no-match rc ≡ tool-fail rc). r25: quality-bar item in the P5 stdlib brief | three-valued verdict; oracle quality bar | — | 16P-DP-3, note-162 | S |
| an-effect-polarity | accumulating `(provider, verb, establish/kill)` tuples; multi-verb without clobber (`apt install` vs `purge`). r23: create/destroy POLARITY retired (opaque values; `!` is exit-code plumbing; ValueClaim{Establish, EstablishInverted, Observe}) | kind-index / gen-kill | — | 16P-T7, note-162, 23F-Add2 | B |
...
| an-version-applicability | derive the *version-correct* `.check`/effects from `install foo@3.5` (version-bearing-fact normalization, API-node resolution) — the MH2 layer. r24: re-justified as the missing tether (rul24-divergence-is-the-game); r25 A1 feeds the unpark | oracle-contract; effect map; MH2 | an-oracle-decls | TODO-ADDTL MH2, 064:19, 24A§1b | O |
...
| an-oracle-decls | per-oracle declarations the engine lifts statically (never sources/runs). r23/24: declarations = the role-sibling FUNCTION BODIES themselves (predict/is_converged/touches/resolve/reaches) + inline annotations; effects derive, markers dead | the whole pipeline | — | 021§0, 16P-T7, 23H, 24G | B |
...
| an-distrust-propagation | a fact derived from an oracle-claimed fact inherits distrust (taint flows through derived facts). Partial: survival/guard witnesses carry licensor attribution; full derived-fact taint open | trust accounting | — | 102, 110-f20 | S |
...
| need | information | needed-by | dual | refs | st |
|---|---|---|---|---|---|
| an-locator-dag | provenance is a per-host-forking N-tier DAG of typed loci, a *variable-length list*, never pre-flattened (composition collapses to coarsest tier). r22: single-tier ProvId arena + ru-11 decision-inert weld BUILT; the N-tier/per-host-forking half unfirmed (first consumer = real hosts; the r25 P3 flat shape is a disclosed ru-26 cut) | error reporting; plan UI; faithful mode | — | 111 dac-A, 110-f30, KNOBS kFIDELITY, 22W§2 | S |
...
| an-render-modes | flat source-ordered render vs line-granular book-faithful render (comments converged lines in place). r24 polish: elision-render greenlit (original-bytes-commented where rc unconsumed) | plan output | an-fidelity-mode | 16P-T14, 16N, 24H | B |
...
| an-marker-protocol | nonce-prefixed colon-delimited status records (`keyword:field:…`, apt `pmstatus` shape); consumed by leaf-runner + provenance-parser + DST-synth + executor — *one shape, 4 seams*. Forced by 22H, not the r25 trial (whole-script floor dodges it) | the physical knot | — | 141-g8, 151-M3/X2 | O |
...
| an-unsafe-boundary | hard ⊤: `eval` / dynamic command-name / `. "$dyn"` / heredoc-codegen / arithmetic-command-position / lvalue-builtins. Trigger-set fixed-in-spirit, shrinks deliberately (loops lowered r21; pipes parse r24) | ⊤-rejection (syntactic) | an-top-surface | 021§2, 055§1A, 16P-T2, spike/CLAUDE | B |
...
| an-apply-3 | targeted desired-set (`dorc try`/`bump`): apply-2 + a *backward* relevance-reduction; `apply-3 ⊃ apply-2`. NB: the future READ-SET consumer — if it ever consumes footprints, re-run the touches() scoping analysis (24A §1b) | hot-loop; kELISION | an-apply-2 | 16P-T13, 16Q q1-backward, 24A§1b | D |
| an-elision-predicate | elide leaf iff `probe=Converged ∧ ambient ∧ Must ∧ no-consumed-unvouched-observable ∧ ¬⊤-contained`; `can't-probe ⇒ can't-elide`. r24: + a REACHED VOUCH conjunct (the elide-weld) + optionally the survival conjunct under --trust-footprints | apply-2 contract | an-probe-vs-run-predicate | 16P-T13, 24C§3B | B |
...
| an-host-as-adversary | a managed host can forge convergence verdicts flowing into elision → silent suppression of a needed apply. Typed emission closed the KIND-minting half (vocabulary closes at lift); values still ride host output by design. Pair with any kSTATE memoization unpark (2026-07-05) | verdict trust; kFAIL-perform | an-verdict-failsafe-default | 150-R10, 151-SF3, 24G§4 | O |
...
| an-wall-topology | plan-time verdict-aware wall walk: a RUNNING unmodeled/unvouched mutator walls downstream; an ELIDED command casts no shadow; a demoted mutator re-walls; total-wall is sticky; kill-walls threaded (kill-coords side-map) | plan verdicts; guard/survival | — | 24A§3 (fd10), 24C§2/§4 | B |
...
| an-footprint-atmost | `touches()`: the at-most WRITE-set, emitted per reached path; emission = the claim, no emission = no claim = wall; write-set only (reads don't kill facts; order-sacred removes anti-deps) | survival tier | an-backing-selfframing | 24A§1b, USER_STORY st.5 | B |
| an-derived-footprint | payload-bound tools ESCALATE: the touches() body ships strip-only into the probe lane, runs read-only, stdout → the footprint (a dynamic frame); all-or-nothing readback; the engine unions the site's own effect-coord into non-empty emissions (own-coord, never manufactured from silence) | survival past payload-bound walls | an-footprint-atmost | 24E, 24C§4, 24G§8 | B |
| an-backing-selfframing | a fact's backing = its own probe's marked read-set, complete by construction (self-framing) — SURVIVAL is settled; the relocated risk is ADEQUACY (converged≠no-op), priced at the vouch | disjointness; risk accounting | an-adequacy-bite | 23N§5, 23O§1.7 | B |
| an-disjointness | footprint ∩ backing = ∅ ⇒ the elision survives a running wall (the separation-logic frame rule; Bernstein's condition); BOTH sides canonicalized through the kind resolver first; argument order type-enforced | survival tier | an-kind-resolver | 23N, 24C§2/§5A | B |
...
| an-kind-reach | `reaches()`: KIND-keyed 1→N closure applied by the engine to EVERY footprint coord of that kind (whoever emitted it); widens claims only — the safe direction; static arms traced/ship-nothing, dynamic arms escalate; footprints only, never backings | cross-author composition | an-kind-resolver | 24G, USER_STORY st.7 | B |
| an-typed-emission | an emitting arm's KIND rides a trailing annotation; entities = raw stdout; the vocabulary CLOSES at lift (a host can never mint a kind at runtime); un-annotated emission MEANS NOTHING (smell, never refusal). touches() migration to this shape owed (sequenced last) | vocabulary hygiene; co-domain statically | — | 24G§4/§5 | B (touches() migration owed) |
| an-mode-gate | the survival tier (elide past a RUNNING wall) sits behind an explicit admin flag, never a default; short un-aliasable name; unflagged behaviour stays silence→run ground truth; "marketing at best, theatre at worst," demanded anyway | admin consent surface | — | 24A§1a | B |
