...
> *Sanctioned exception:* exactly TWO comment-parses exist — the dialect-version marker (`# dorc-lang/v0.2`, first ~10 lines) and the `#:` mark carrier (immediate-colon, a closed strip-if-valid grammar carrying the same strip-only marks the colon form does); both gate syntax only, never `__role` name-recognition (a permanent, unversionable surface), and neither is sidecar *configuration*. Eol-comment annotations otherwise stay rejected. (The `24M` one-parse limit was LIFTED by the `281` §2j human ack that sanctioned `#:`; rul24M-version-comment, `Research/notes/24M` §1.)
...
**Tension:** how a non-sh-native **type-annotation** is spelled in the script. `kTYANNOT-inline` annotates directly on a command argument — ergonomic, intuitive, significant-meaning-in-place — but is *not inert* under stock shells (aborts or silently corrupts; verified, `Research/plans/17O` F-OFFRAMP), so it demands a correctness-critical strip pass, taxing `kLANG`'s "absolutely trivial" off-ramp. `kTYANNOT-eol-comment` carries it on the `#:` comment carrier — inert under any shell (off-ramp-free) and, since the `281` §2j human ack, a *sanctioned* second comment-parse rather than a `kOOB` breach; its residual costs are highlight-demotion (the leading `#` greys the mark) and forcing typed values out of argument-position. *Both poles sacrifice a "spelled in sh" principle*; the knob is *which*.
**Status:** directional — **both poles ship: `kTYANNOT-inline` (the salient colon form) is the DEFAULT, and `kTYANNOT-eol-comment` (the `#:` carrier) is OFFERED for raw-exec-inertness; the formal weld is human-reserved.** The upstream gate (`dq-kOOB`: whether a type-surface exists at all) resolved by construction: the inline dialect (bare `__role`-named functions, inline binds, trailing marks) is stamped and implemented, with the off-ramp cost paid by the strip pass (bare marks delete whole-statement; the author's last substantive command stays the last status-affecting statement). Two containments cheapen the trade: annotations are **marker-gated per-file** (a book is always plain sh -- the marker and dorc-lang live only in oracle files, which erase to plain sh via `dorc strip`), and function names are bare munged POSIX NAMEs, so the strip is pure bind/mark erasure and rewrites no names (rul24M-bare-dorcism-names · rul24-totalistic-munge, `Research/notes/24C`). The eol pole is now clean too: `#:` is a sanctioned comment-parse (`281` §2j / `KNOBS:kOOB`), inert on every route even unstripped, its only cost highlight-demotion. Residual `dq-kOOB` scope: the formal stamp (human's); entity/selector grammar + per-position charsets (human-deferred; the selector introducer landed as `@`, permanent at v0.2 grade — `kSALIENCE`); kind evolution beyond the within-unit duplicate-resolver refusal (rul24M-kind-unify-owed). Bootstrap-kind curation: resolved — `sm.dorc.*`, mandatory reverse-DNS ≥2 dots.
...
**Owner:** user (DX/taste) + corpus (what actually highlights). **Lock-in:** med — the choice threads the parser and every marked oracle; salience characters are cheap to grant, near-impossible to claw back. `@` inherits the selector-introducer's permanent-going-forward status at **v0.2 grade** — a non-`@` selector would be a v0.3 event (`281` §2j).
> Entangled with `kTYANNOT` (the annotation spelling this constrains — inline-vs-comment is orthogonal to salient-vs-invisible) and `kLANG`/`kOOB` (the invisibility pole's welds). Source: `Research/plans/281` §10 (salience goal) / §R4 (`@` over `#`); `Research/plans/27U` §10 (predecessor design).
...
### `kBACKFLIPS`
Poles: `kBACKFLIPS-verbatim-or-refuse ↔ kBACKFLIPS-compile-to-fit`

**Tension:** `kFLATTEN`'s sibling at plan EMISSION. `kFLATTEN` is how much flattening Dorc does for *itself* at probe time; this is how far Dorc will go, *at the admin's request*, to produce an emission form the authored book does not naturally admit (single-stream above all). `kBACKFLIPS-verbatim-or-refuse`: the plan is the user's bytes, relocated only where relocation is provably identity; a form the book cannot take is refused by name, with the form that works. `kBACKFLIPS-compile-to-fit`: source-to-source transformation to make the form possible, buying coverage at the price of a plan nobody authored and a second shell semantics to get right.
**Status:** **WELDED (human, typed 2026-08-22) → `kBACKFLIPS-verbatim-or-refuse`**; the floor is deliberately uneven across forms (multipart accepts strictly more book code than single-stream), and the most a construct may cost its author is single-stream, never support. The fuzzy line is an enumerated list of sanctioned edit classes — `plans/30P` carries it and the refused set. **Owner:** welded (human). **Lock-in:** high.
> Entangled: `kFLATTEN` (sibling, probe-side) · `kLANG`/`kOOB` (the off-ramp welds this protects) · `kBOOT` (single-stream doubles as the no-writable-fs transport floor).
> Scope: the weld binds FORM. The sanctioned edit classes reach inside descent-authorized payloads (a heredoc or argv remainder an oracle has declared to be sh) exactly as they apply to the outer script (`26Ob:rul-descent-implies-manipulation`); fusing N argv-form transits into one session is a form conversion and stays refused.
...
Standing fences while parked: receipts are write-only postmortem durables, never this knob's reuse-cache — nothing re-ingests them across runs into a decision; read-back is report-only (rec-5, welded; `spike/CLAUDE.md` receipts-not-a-cache); and the wall-clock-keyed verdict classes (package-index freshness, cert expiry) are the first hermeticity test-cases any content-key design must survive — they are inherently volatile-keyed and break naive keying first.
...
### `kBOOT`
Poles: `kBOOT-ssh-assumed <-> kBOOT-any-byte-pipe`

**Tension:** engineering bent toward the sshd-up steady state -- one adjudicated transport spec (`260` §5), OpenSSH's own identity/auth machinery, channel assumptions baked once -- **vs** reach across the whole byte-pipe floor: every channel that eventually feeds bytes to a POSIX sh (container-exec, hypervisor guest-agents, cloud exec-APIs, installer hooks, one-shot user-data payloads, spliced fragments inside other tools' generated scripts), across a machine's entire lifecycle including pre-sshd birth. (The name under-describes deliberately: the axis covers degraded channels on *mature* machines -- `docker exec`, SSM -- as much as literal boot.)
**The floor, stated precisely (the high-lock-in half):** channel capabilities are *independent*, not ranked -- adbd's source carries the 2x2 literally (rc-visibility independent of stream-separation; a PTY forecloses separation even with framing), and adb/k8s-exec both paid a protocol v2 to recover the exit code Dorc's in-band sentinel refuses to need (on Azure's classic run-command, which reports *no* rc at all, the sentinel is required, not defensive). Today's true assumed floor is {byte-clean stdin, separate stderr, non-echoing} -- which is exactly Ansible's *pipelining* cell, shipped disabled there because sudo/`requiretty` conflicts with stdin-fed payloads: privilege escalation is the named thing that historically breaks this cell. The floor is *asymmetric*: byte-clean outbound is assumed; a byte-clean *return* path never is (SSM is UTF-8-only; `ansible raw` advises base64) -- intake law already treats return bytes as bounded untrusted raw. Under degradation the *report lane* breaks first (exec-API output caps are sized for human eyes, not drains; AWS truncates keeping the head, Azure keeping the tail -- sentinel *placement* is a per-channel constraint), probe survives wherever any return path exists, and only the eventual live topology (`142`) genuinely needs a file side-channel -- with prior-art bridges (`dd`-over-exec, in-band framing) all the way down, the single-channel whole-artifact shape stays a supported floor forever, not scaffolding. Rc nuance both directions: never *trust* channel rc != never *produce* a meaningful rc (installer `late-commands` legitimately consumes the artifact's).
**Resolution mechanism (human-typed, this round):** per-feature capability matching -- Dorc probes capabilities per-host, per-feature, and provides the full feature-set each host's capabilities license; never a tiered degradation ladder (target populations are non-monotonic: busybox `setsid` without `--wait`, Alpine `timeout` without `-t`, DNF5 below DNF3 -- the web's feature-detection-not-user-agent-sniffing lesson). Below the engineered floor sit two ruled rungs: human-mediated paste (a book `printf`s a block and waits -- always available, never probed, inherently outside the attention product) and the splice floor (fragments compiled to live inside *other tools' generated scripts* -- never the core target, but a floor to maintain and watch other work damage, since it needs paste's properties *plus* purity/safety/defensive compilation).
**Status:** directional -- lean `kBOOT-any-byte-pipe` **at the floor-definition tier now**; mechanisms deferred (the floor assumptions are the retrofit-hostile part; the machinery that reaches the floor consistently is low-lock and waits for the transport v2). Delivery route RULED 2026-09-03 (`26N:rul-delivery-shape-file-backed-default`): writable scratch is a probed per-context capability selecting the route -- file-backed by default, in-memory as the measured floor -- and the capability system that grows from it is `Research/notes/26N` §4; link and context capabilities as distinct axes, and the matrix that fills over time, are `26O:2-contexts-links-and-capabilities`. **Owner:** user + field evidence (round `ops-glue-residue`, 2026-07-28). **Lock-in:** high on floor assumptions and sentinel doctrine; low on per-channel mechanisms.
> False friend of `kCOMMS` (the form of Dorc's own controller<->host signalling *given* a channel; this knob is which channels exist at all) and `kTPLATFORMS` (which targets we bend for; this is how we reach them). Gated by `kLANG` (the sh-precondition bounds all reach); rides `kAGENTLESS`. Evidence base: `.claude/research/ops-glue-residue/` (charter + turn02/turn04 notes, 183-source graded ledger); plan + synthesis: `Research/plans/26K`.
...
**Welded, phase-keyed**: the probe phase fails `kFAIL-withhold`, the apply phase `kFAIL-perform` — opposite safe directions, not a dial. The one thing never traded for performance. Refined framing, human-typed 2026-09-03 (`26N:rul-dorc-acts-are-withhold-shaped`): any Dorc act not provably caused by the author's code executing naturally is withhold-shaped; only explicitly-authored, unmodified lines Dorc can Must-conclude would have run under plain sh are perform-shaped. **Owner:** welded. **Lock-in:** absolute.
