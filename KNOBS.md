...
**Status:** directional -- lean `kBOOT-any-byte-pipe` **at the floor-definition tier now**; mechanisms deferred (the floor assumptions are the retrofit-hostile part; the machinery that reaches the floor consistently is low-lock and waits for the transport v2). Delivery route RULED 2026-09-03 (`26N:rul-delivery-shape-file-backed-default`): writable scratch is a probed per-context capability selecting the route -- file-backed by default, in-memory as the measured floor -- and the capability system that grows from it is `Research/notes/26N` §4; link and world capabilities as distinct axes, and the matrix that fills over time, are `plans/26O` §2. **Owner:** user + field evidence (round `ops-glue-residue`, 2026-07-28). **Lock-in:** high on floor assumptions and sentinel doctrine; low on per-channel mechanisms.
