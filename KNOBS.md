...
**Tension:** `kFLATTEN`'s sibling, one phase later. `kFLATTEN` is how much flattening Dorc does for *itself* at probe time — internal, invisible, cheap to get wrong. This knob is how far Dorc will go, *at the admin's request*, to produce an emission form the authored book does not naturally admit — single-stream above all (one file: pipeable, reviewable as a whole, and the transport floor for hosts with no writable directory, `kBOOT`). `kBACKFLIPS-verbatim-or-refuse`: the plan is the user's bytes, relocated only where relocation is provably identity; a requested form the book cannot take is REFUSED pre-network, naming the construct and the form that works (multipart) — "what you approved is exactly what executes" stays literally true and the off-ramp stays trivial. `kBACKFLIPS-compile-to-fit`: source-to-source transformation to make the form possible — generated loader functions, rewritten `return`s, re-scoped positional parameters, synthesized dispatch — buying "one file for any book" at the price of a plan nobody authored, a second shell semantics Dorc must get right across every target, and a user-story that now contains "Dorc rewrote your book".
**Status:** **WELDED (human, typed 2026-08-22) → `kBACKFLIPS-verbatim-or-refuse`.** Not every book admits every emission form, and the floor is deliberately uneven: multipart output accepts strictly more book code than single-stream, and the MOST a construct may cost its author is single-stream — never support (the test-then-source idiom and `return`-guarded library files are the motivating cases). The fuzzy line ("what counts as a rewrite?") is made crisp by enumeration: exactly three sanctioned edit classes exist, and a fourth is a loud human act, never a lane's. (1) Import re-say in GENERATED plans — the one book-byte rewrite (`30Ng:rul-bundle-at-dorc-lang-boundaries`: a generated plan is a durable, not an off-ramp durable). (2) Header-only renames of ORACLE-custody definitions in shipped artifacts (`28R:rul-munge-oracle-names-only`; book bytes never). (3) Byte-verbatim relocation — paste in place, front-lift — under a floor-measured identity set (`floor30-inline-dot-boundary`; the front-lift ladder `30Ng:bundle-front-lift-ladder`; the plain-sh paste exclusion set the inclusion design owes). Refused by name, permanently: generated loader functions (floor-REFUTED, `30I:pin-one-file-root-bundle`), any rewrite of a book's control flow, positional parameters, `return`/`exit`, or scoping, and synthesized dispatch. Re-opening is the opposite of gradual: it stands between the human and the product having *no* residual value, in extremis only. **Owner:** welded (human). **Lock-in:** high — the artifact forms, the refusal posture, `rul-piped-stdout-carries-a-full-plan`, and `two-surfaces` all assume it.
> Entangled: `kFLATTEN` (sibling, probe-side) · `kLANG`/`kOOB` (the off-ramp welds this protects) · `kBOOT` (single-stream doubles as the no-writable-fs transport floor, so a construct that costs single-stream also costs those hosts — honestly, by name). Naming note: `kLOWERING` was declined as ambiguous; the poles say what is actually being risked.
